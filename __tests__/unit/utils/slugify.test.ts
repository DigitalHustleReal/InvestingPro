...
describe("slugifyTerm", () => {
  ...
  it("replaces multiple non-alphanumeric characters with a single hyphen", () => {
    expect(slugifyTerm("Best Credit Cards -- 2026")).toBe(
  ...
  it("handles special characters and punctuation", () => {
    expect(slugifyTerm("SBI Card: Rewards & Benefits!")).toBe(
      "sbi-card-rewards-benefits",
    );
    expect(slugifyTerm("HDFC 10X Rewards (2026)")).toBe(
      "hdfc-10x-rewards-2026",
    );
  });
