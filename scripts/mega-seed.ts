...
function generateCreditCard(index: number) {
  ...
  return {
    ...
    features: {
      ...
      welcome_bonus:
        annualFee > 2000
          ? `${getRandomInt(5000, 25000)} bonus points`
          : undefined,
    },
    pros: [
      "High reward rate on spends",
    ...
    cons:
      annualFee > 0
        ? ["Annual fee applicable", "Income requirement"]
...
function generateLoan(index: number) {
  ...
  const interestRate =
    loanType === "Home Loan"
      ? getRandomFloat(8.5, 9.5)
      : loanType === "Car Loan"
        ? getRandomFloat(8.0, 10.5)
  ...
  return {
    ...
    features: {
      ...
      max_amount:
        loanType === "Home Loan"
          ? `₹${getRandomInt(50, 500)} Lakhs`
          : `₹${getRandomInt(5, 50)} Lakhs`,
...
function generateMutualFund(index: number) {
  ...
  return {
    ...
    features: {
      ...
      risk_level:
        fundType.includes("Small") || fundType.includes("Midcap")
          ? "Very High"
          : "Moderately High",
...
function generateInsurance(index: number) {
  ...
  return {
    ...
    features: {
      cover_amount: isHealth
        ? `₹${getRandomInt(3, 25)} Lakhs`
        : `₹${getRandomInt(50, 200)} Lakhs`,
      premium: isHealth
        ? `₹${getRandomInt(8, 25)}K/year`
        : `₹${getRandomInt(5, 15)}K/year`,
...
function generateFixedDeposit(index: number) {
  ...
  return {
    ...
    rating: getRandomFloat(4.0, 4.8),
    features: {
      interest_rate: isBankFD
        ? `${getRandomFloat(6.5, 7.5)}% p.a.`
        : `${getRandomFloat(7.5, 9.0)}% p.a.`,
    ...
    pros: [
      "Guaranteed returns",
      "Flexible tenure",
      isBankFD ? "DICGC insured" : "Higher interest rates",
...
function generateDematAccount(index: number) {
  ...
  return {
    ...
    cons: [
      "Intraday charges apply",
      broker === "Zerodha" ? "AMC of ₹300/year" : "Limited research tools",
    ],
...
function generatePPFNPS(index: number) {
  ...
  return {
    ...
    pros: isPPF
      ? [
          "100% safe - Govt backed",
          "Tax-free interest",
          "EEE status - fully tax exempt",
...
// --- MAIN SEED FUNCTION ---
async function seed() {
  // ENVIRONMENT GUARD: Block running in production
  const baseUrl =
  ...
  if (
    ...
    console.error(
      "SAFETY ABORT: This seed script generates dummy/random data.",
    );
    console.error("It should NEVER be run against a production database.");
    console.error(
  ...
  for (let i = 0; i < brokers.length; i++)
    products.push(generateDematAccount(i));
  ...
  for (let i = 0; i < products.length; i += chunkSize) {
    ...
    if (error) {
      console.error(
        `❌ Chunk ${Math.floor(i / chunkSize) + 1} failed: ${error.message}`,
      );
      failed += chunk.length;
    } else {
      console.log(
        `✅ Chunk ${Math.floor(i / chunkSize) + 1} - ${chunk.length} products inserted`,
      );
