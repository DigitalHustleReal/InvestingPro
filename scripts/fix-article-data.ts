...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  ...
  console.error(
    "   Set them in .env.local or via your shell before running this script.",
  );
  process.exit(1);
}
...
async function fixArticleData() {
  console.log(
    "🔍 Checking for published articles with missing dates or images...",
  );
  ...
  if (!articles || articles.length === 0) {
    ...
    const categoryImages: Record<string, string> = {
      investing:
        "https://images.unsplash.com/photo-1579532537598-459ecdaf39cc?auto=format&fit=crop&q=80&w=2070",
      "credit-cards":
        "https://images.unsplash.com/photo-1556742049-0cfed4f7a07d?auto=format&fit=crop&q=80&w=2070",
      loans:
      ...
      default:
        "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=2015", // generic reporting
    };
    ...
    for (const article of articles) {
      ...
      if (!article.featured_image) {
        ...
        updates.featured_image =
          categoryImages[cat] || categoryImages["default"];
      }
      ...
      if (updateError) {
        console.error(
          `❌ Failed to fix "${article.title}":`,
          updateError.message,
        );
      } else {
        console.log(
          `✅ Fixed "${article.title}" (Date: ${!!updates.published_at}, Image: ${!!updates.featured_image})`,
        );
      }
    }
  }
