...
import { parse, isSameDay, isBefore, subYears } from "date-fns";
...
function getNavAtDate(data: MFData[], date: Date): number {
  for (const entry of data) {
    const entryDate = parse(entry.date, "dd-MM-yyyy", new Date());
    if (isSameDay(entryDate, date) || isBefore(entryDate, date)) {
...
async function processScheme(scheme: MFScheme) {
  try {
    // Fetch details
    const { data: details } = await axios.get<MFDetails>(
      `${MFAPI_BASE}/${scheme.schemeCode}`,
    );
    ...
    const lastDate = parse(navHistory[0].date, "dd-MM-yyyy", new Date());

    // Calculate Returns
    const nav1Y = getNavAtDate(navHistory, subYears(lastDate, 1));
    const nav3Y = getNavAtDate(navHistory, subYears(lastDate, 3));
    const nav5Y = getNavAtDate(navHistory, subYears(lastDate, 5));
    ...
    if (nameLower.includes("large cap") || nameLower.includes("bluechip"))
      ...
      category = "Index Fund";
    else if (nameLower.includes("debt") || nameLower.includes("liquid"))
      category = "Debt";
