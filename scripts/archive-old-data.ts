...
function getS3Client(): any {
    ...
    if (!accessKeyId || !secretAccessKey || !bucket) {
        logger.warn('S3 not configured - archival will be skipped. Set AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and AWS_S3_ARCHIVE_BUCKET');
        return null;
    }
...
async function archiveOldArticles(
    ...
    try {
        ...
        cutoffDate.setDate(cutoffDate.getDate() - RETENTION_POLICIES.articles.archiveAfterDays);
        ...
        if (s3Client && process.env.AWS_S3_ARCHIVE_BUCKET) {
            ...
            logger.warn('S3 not configured - articles will be marked as archived but not backed up');
        }
        ...
        logger.error('Failed to archive articles', error instanceof Error ? error : new Error(errorMessage));
    }
...
async function deleteOldAnalytics(
    ...
    try {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - RETENTION_POLICIES.analytics.deleteAfterDays);
        ...
        logger.info(`Deleted analytics data older than ${RETENTION_POLICIES.analytics.deleteAfterDays} days`);
        result.count = 1; // Approximate count
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        result.errors.push(errorMessage);
        logger.error('Failed to delete old analytics', error instanceof Error ? error : new Error(errorMessage));
    }
...
async function archiveOldWorkflows(
    ...
    try {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - RETENTION_POLICIES.workflows.archiveAfterDays);
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        result.errors.push(errorMessage);
        logger.error('Failed to archive workflows', error instanceof Error ? error : new Error(errorMessage));
    }
...
async function deleteOldSystemEvents(
    ...
    try {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - RETENTION_POLICIES.systemEvents.deleteAfterDays);
        ...
        logger.info(`Deleted ${count} system events older than ${RETENTION_POLICIES.systemEvents.deleteAfterDays} days`);
        result.count = count;
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        result.errors.push(errorMessage);
        logger.error('Failed to delete old system events', error instanceof Error ? error : new Error(errorMessage));
    }
...
export async function runArchival(): Promise<{
    ...
    try {
        ...
        logger.error('Archival process failed', error instanceof Error ? error : new Error(String(error)));
        return {
            success: false,
            results,
            summary: {
