...
export function MethodologyLink({ variant = 'text', className = '' }: MethodologyLinkProps) {
    if (variant === 'button') {
        return (
            <Link
                href="/methodology"
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-stone-700 dark:text-slate-300 text-sm font-medium transition-colors ${className}`}
            >
