 ...
 */
export default function InlineSignup({ variant, cardName, category, className }: InlineSignupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email) return;
    setLoading(true);
    // Simulate API call — replace with actual Supabase/Resend integration
  ...
  if (submitted) {
    return (
      <div className={cn("bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-xl p-5 text-center", className)}>
        <CheckCircle2 className="w-8 h-8 text-green-600 dark:text-green-400 mx-auto mb-2" />
        <p className="font-bold text-green-800 dark:text-green-300 text-sm">You&apos;re all set!</p>
        <p className="text-xs text-green-600 dark:text-green-500 mt-1">
          {variant === 'card-alerts' && `We'll notify you about ${cardName || 'card'} updates and better alternatives.`}
          {variant === 'save-results' && 'Your comparison results have been saved. Check your inbox.'}
          {variant === 'weekly-picks' && 'Expect your first weekly picks email this Friday.'}
        </p>
      </div>
    );
  }
  ...
  const configs = {
    ...
    'save-results': {
      ...
      description: 'Email yourself these results to review later. No account needed.',
      placeholder: 'your@email.com',
      cta: 'Email Results',
      bgClass: 'bg-green-50/50 dark:bg-green-950/10 border-green-200 dark:border-green-800',
    },
    'weekly-picks': {
