...
const CATEGORY_OFFERS: Record<string, CategoryOffer> = {
  'credit-cards': {
    ...
    description: 'Get our expert-curated list of top credit cards for rewards, cashback, travel & more.',
  ...
  'loans': {
    ...
    description: 'Compare rates from 20+ banks. Our guide shows you how to get the lowest EMI.',
  ...
  'demat-accounts': {
    ...
    description: 'Compare brokers, understand charges, and open your first demat account.',
  ...
  'calculators': {
    ...
    description: 'Get your calculation results with projections and recommendations sent to your inbox.',
  ...
  'default': {
    ...
    subtitle: 'Join 25,000+ readers',
    description: 'Get our best money-saving tips and product updates every Tuesday.',
...
export function ContextualLeadMagnet({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('ip_dismissed_offers', dismissed + ',' + offer.guideType);
    } catch (error) {
  ...
  if (variant === 'banner') {
    return (
      <div className={`bg-gradient-to-r ${offer.color} rounded-xl p-4 text-white relative`}>
        <button
  ...
  return (
    <div className={getVariantClasses(variant)}>
      ...
      {/* Header */}
      <div className={`bg-gradient-to-br ${offer.color} p-4 -mx-4 -mt-4 mb-4 rounded-t-xl`}>
        <div className="flex items-center gap-3">
...
function detectCategory(pathname: string): string {
  ...
  if (pathname.includes('/fixed-deposit') || pathname.includes('/fd')) return 'fixed-deposits';
  if (pathname.includes('/demat') || pathname.includes('/stock')) return 'demat-accounts';
