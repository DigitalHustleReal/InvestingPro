...
export default function ComparisonOutcomeWidget({ products }: ComparisonOutcomeWidgetProps) {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-lg mb-8">
            ...
            <div className="p-6 grid md:grid-cols-2 gap-8 relative">
                {/* Visual Separator */}
                <div className="absolute left-1/2 top-4 bottom-4 w-px bg-slate-100 dark:bg-slate-800 hidden md:block"></div>

                {/* Winner 1 Breakdown */}
                <div>
                   <div className="flex items-center gap-3 mb-4">
                       {/* Image placeholder or Provider Logo */}
                       <div className="bg-slate-100 dark:bg-slate-800 w-12 h-8 rounded flex items-center justify-center text-xs font-bold text-slate-500">
                          {p1.provider_name?.substring(0,3)}
                       </div>
                       <h4 className={`font-bold text-lg ${overallWinner?.id === p1.id ? 'text-primary-700 dark:text-primary-400' : 'text-slate-700 dark:text-slate-300'}`}>
                ...
                <div>
                   <div className="flex items-center gap-3 mb-4">
                       ...
                       <h4 className={`font-bold text-lg ${overallWinner?.id === p2.id ? 'text-primary-700 dark:text-primary-400' : 'text-slate-700 dark:text-slate-300'}`}>
