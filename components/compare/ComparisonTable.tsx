    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            {rows.map((row, idx) => (
                <div
                    key={row.label}
                    className={`grid grid-cols-1 md:grid-cols-4 border-b border-gray-100 last:border-b-0 ${row.highlight ? 'bg-emerald-50/30' : idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'
                        }`}
                >
                        ...
                        return (
                            <div key={product.id} className="p-4 text-sm text-gray-900 border-r border-gray-200 last:border-r-0 flex items-center">
                                {value !== undefined && value !== null ? (
                                    <span className={row.highlight ? 'font-bold text-emerald-600 text-base' : ''}>
                                        {value}
                                    </span>
                                ) : (
