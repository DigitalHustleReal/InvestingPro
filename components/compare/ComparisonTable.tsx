    ...
    if (productType === "credit_card") {
      ...
      return [
        ...
        },
        {
          label: "Interest Rate",
          key: "interestRate",
          format: (p: any) => `${p.interestRateMin}% - ${p.interestRateMax}%`,
          winner: "min",
        },
      ...
      return [
        ...
        },
        {
          label: "3Y Returns",
          key: "returns3Y",
          format: (v: any) => formatPercentage(v),
          highlight: true,
          winner: "max",
        },
        {
        ...
        },
        {
          label: "Risk Level",
          key: "riskLevel",
          format: (v: any) => v.replace("_", " ").toUpperCase(),
        },
      ...
      <div className="bg-card dark:bg-gray-900 rounded-xl shadow-lg border border-border dark:border-gray-800 overflow-hidden min-w-[800px]">
        ...
        <div className="grid grid-cols-4 bg-muted/50 border-b border-border">
          ...
          {products.map((product) => (
            <div
              key={product.id}
              className="p-6 relative border-r border-border last:border-r-0"
            >
              {onRemove && (
                <button
              ...
              <div className="flex items-center gap-2 mb-2">
                <div className="flex items-center bg-accent/10 px-2 py-0.5 rounded text-accent-foreground text-xs font-bold border border-accent/20">
                  {product.rating}{" "}
                  <Star className="w-3 h-3 ml-0.5 fill-accent text-accent" />
                </div>
                {product.isPopular && (
        ...
        {/* Comparison Rows */}
        {rows.map((row, idx) => {
          const winnerId = row.winner
            ? getWinnerId(row.key, row.winner as "max" | "min")
            : null;
            ...
            >
              ...
              {products.map((product) => {
                ...
                return (
                  <div
                    key={product.id}
                    className={`p-4 text-sm border-r border-border last:border-r-0 flex items-center ${isWinner ? "bg-success/10 font-semibold" : "text-card-foreground"}`}
                  >
          ...
          <>
            <div className="grid grid-cols-4 border-b border-gray-100 dark:border-gray-800 bg-emerald-50/30 dark:bg-emerald-900/10">
              ...
              {products.map((product) => (
                <div
                  key={product.id}
                  className="p-4 text-xs text-card-foreground border-r border-border last:border-r-0"
                >
        ...
        <div className="grid grid-cols-4 bg-muted/30">
          ...
          {products.map((product) => (
            <div
              key={product.id}
              className="p-4 border-r border-gray-100 dark:border-gray-800 last:border-r-0"
            >
              ...
              <p className="text-[10px] text-gray-400 dark:text-gray-500 text-center mt-1">
                Secure application · Takes 3 min
              </p>
            </div>
          ))}
        </div>
