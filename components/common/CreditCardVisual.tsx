...
> = {
  ...
  "SBI Card": {
    bg: "from-blue-800 to-emerald-600",
...
export default function CreditCardVisual({
  ...
  const theme = BANK_THEMES[bankName] || {
    bg: "from-gray-700 to-gray-500",
    accent: "text-emerald-300",
    text: "text-white",
  };
  const typeLabel = cardType
    ? TYPE_LABELS[cardType] || cardType.toUpperCase()
    : "";
  ...
  return (
    ...
    >
      ...
      <div
        className={cn("font-mono tracking-widest mb-2 opacity-70", theme.text)}
      >
        •••• •••• •••• ••••
      </div>
      ...
      >
        {cardName.replace(bankName, "").replace("Credit Card", "").trim() ||
          cardName}
      </div>
      ...
      <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between">
        <span
          className={cn(
            "font-bold uppercase tracking-wider opacity-80",
            theme.text,
