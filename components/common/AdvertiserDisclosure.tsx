...
export function AdvertiserDisclosure({ className = '' }: { className?: string }) {
    return (
        <div className={`flex items-start gap-2 p-3 bg-slate-50/50 border border-slate-100 rounded-lg text-xs text-slate-500 ${className}`}>
            <Info className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-slate-400" />
            <p>
                <span className="font-semibold text-slate-600">Advertiser Disclosure:</span> InvestingPro.in is an independent comparison platform. 
