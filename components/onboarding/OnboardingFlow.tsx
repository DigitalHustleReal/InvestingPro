          ...
          <div className="lg:col-span-2 bg-gray-900 p-8 text-white hidden lg:flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-6">
              {steps.map((s, i) => (
                <div
                  key={i}
                  className={`flex items-center gap-4 transition-all duration-300 ${i === currentStep ? "opacity-100 translate-x-1" : i < currentStep ? "opacity-50" : "opacity-20"}`}
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold border ${i === currentStep ? "bg-white text-gray-900 border-white" : "border-white/50"}`}
                  >
                    {i < currentStep ? (
          ...
          <div className="lg:col-span-3 p-10 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-center mb-10">
                <Progress
                  value={progress}
                  className="h-1.5 flex-1 mr-4 bg-gray-100"
                />
                ...
                {currentStep === 0 && (
                  <div className="space-y-4">
                    <p className="text-sm text-gray-600 font-medium">
                      Click "Get Started" to begin your personalized financial
                      audit.
                    </p>
                  </div>
                )}
                    ...
                    {[
                      {
                        ...
                        sub: "High risk, multi-bagger potential",
                      },
                      {
                        id: "income",
                        label: "Passive Income Stream",
                        sub: "Dividends & consistent yield",
                      },
                      {
                        id: "preservation",
                        label: "Capital Preservation",
                        sub: "Safety first, inflation beating",
                      },
                    ].map((opt) => (
                      <button
                        key={opt.id}
                        onClick={() =>
                          setFormData({ ...formData, goal: opt.id })
                        }
                    ...
                    {[
                      {
                        id: "conservative",
                        label: "Conservative",
                        sub: "Low volatility, stable bonds/FDs",
                      },
                      {
                        id: "moderate",
                        label: "Moderate",
                        sub: "Mix of equity and debt",
                      },
                      {
                        id: "aggressive",
                        label: "Aggressive",
                        sub: "Focus on small/mid caps & tech",
                      },
                    ].map((opt) => (
                      <button
                        key={opt.id}
                        onClick={() =>
                          setFormData({ ...formData, risk: opt.id })
                        }
                        className={`p-4 rounded-xl border-2 text-left transition-all ${
                          formData.risk === opt.id
                            ? "bg-primary-50 border-primary-500 shadow-lg shadow-primary-500/5"
                            : "bg-white border-gray-100 hover:border-gray-200"
                        }`}
                      >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[
                      ...
                      <button
                        ...
                        className={`p-4 rounded-xl border-2 text-center transition-all flex flex-col items-center gap-3 relative ${
                      ...
                      >
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center ${formData.interests.includes(opt.id) ? "bg-accent-500 text-white" : "bg-gray-50 text-gray-600"}`}
                        >
                          <opt.icon className="w-5 h-5" />
                        </div>
                        <span className="font-bold text-[10px] uppercase tracking-widest text-gray-900">
                          {opt.label}
                        </span>
                        {formData.interests.includes(opt.id) && (
