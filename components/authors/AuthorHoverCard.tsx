...
function normalizeCredentials(credentials: unknown): string[] {
  if (Array.isArray(credentials))
    return credentials.filter((c) => typeof c === "string");
...
export function AuthorHoverCard({
  ...
  const imageUrl =
    author.photo_url ||
    `https://api.dicebear.com/7.x/initials/svg?seed=${author.slug}`;
    ...
    <HoverCard>
      <HoverCardTrigger asChild>
        <Link
          ...
          className={cn(
            "cursor-pointer hover:text-primary-600 hover:underline decoration-primary-600/30 underline-offset-4 transition-colors flex items-center gap-2",
            className,
          )}
        >
          {showAvatar && (
      ...
      <HoverCardContent
        className="w-80 md:w-96 p-0 overflow-hidden border-gray-200 dark:border-gray-800 shadow-2xl rounded-2xl"
        sideOffset={10}
      >
        {/* Header with Photo */}
            ...
            {safeCredentials.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {safeCredentials.slice(0, 2).map((cred, i) => (
                  <span
                    key={i}
                    className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 shadow-sm"
                  >
                    {cred}
                  </span>
                ))}
        ...
        <div className="px-6 pb-6 pt-0 bg-gradient-to-b from-gray-50/50 to-white dark:from-gray-900/50 dark:to-gray-950">
          <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-3 leading-relaxed">
            {author.bio ||
              author.short_bio ||
              `Expert personal finance researcher and editor at InvestingPro.`}
          </p>
        </div>
          ...
          <div className="flex gap-1">
            {author.social_links?.twitter && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-white dark:bg-gray-800 text-gray-600 hover:text-blue-500 hover:shadow-md transition-all"
              >
            ...
            {author.social_links?.linkedin && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-white dark:bg-gray-800 text-gray-600 hover:text-blue-600 hover:shadow-md transition-all"
              >
...
export function AuthorHoverCardCompact({ author }: { author: Author }) {
  return (
    <AuthorHoverCard
      author={author}
      className="text-gray-900 dark:text-white font-semibold hover:text-primary-600"
    />
  );
}
