...
export default function FundStructuredData({
  fund,
  slug,
  faqs,
}: FundStructuredDataProps) {
  ...
  const financialProductSchema = {
    ...
    // AggregateRating removed — no real user reviews exist yet.
    // Google policy requires ratingCount based on actual user reviews, not fabricated numbers.
    additionalProperty: [
      {
        "@type": "PropertyValue",
        name: "NAV",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: fund.category,
        item: `${baseUrl}/mutual-funds?category=${encodeURIComponent(fund.category)}`,
      },
