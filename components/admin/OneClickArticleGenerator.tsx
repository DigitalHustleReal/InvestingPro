...
export default function OneClickArticleGenerator() {
  ...
  const [step, setStep] = useState<
    "input" | "generating" | "review" | "publishing"
  >("input");
  ...
  const generateArticle = async () => {
    ...
    if (runInBackground) {
      try {
        ...
        toast.success(
          "Article generation queued! Check the Automation tab to track progress.",
        );
        reset(); // Reset form
      } catch (error: any) {
...
- Word count must match content length requirement`;
      ...
      if (data.success && data.article) {
        ...
        const hasContent =
          ...
          (data.article.structured_content &&
            data.article.structured_content.sections?.length > 0);

        if (!hasContent) {
          throw new Error(
            "Generated article has no content. Please try again.",
          );
        }
        ...
        setStep("review");
        toast.success(
          "Article generated successfully! Review and edit if needed.",
        );
      } else {
      ...
      toast.error(
        `Generation failed: ${error.message || "Unknown error. Please check your OpenAI API key and try again."}`,
      );
      setStep("input");
    } finally {
      setGenerating(false);
    ...
    try {
      ...
      const article = await api.entities.Article.create({
        ...
        excerpt:
          ...
          generatedArticle.meta_description?.substring(0, 150),
        seo_title: generatedArticle.seo_title || generatedArticle.title,
        seo_description:
          generatedArticle.meta_description || generatedArticle.excerpt,
  ...
  return (
    <div className="space-y-6">
      ...
      {step === "input" && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-wt-text-muted/50 dark:text-wt-text-muted/50 mt-2">
              Generate complete, SEO-optimized articles in one click. Perfect
          ...
          <CardContent className="space-y-6">
                ...
                <SelectContent>
                  {categories.length > 0 ? (
                    ...
                    <>
                      ...
                      <SelectItem value="investing-basics">
                        Investing Basics
                      </SelectItem>
                    </>
                  )}
            ...
            <div className="flex items-center space-x-2 py-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setRunInBackground(checked as boolean)
                }
              />
              <div className="grid gap-1.5 leading-none">
                ...
                <p className="text-xs text-wt-text-muted/70 dark:text-wt-text-muted/70">
                  Recommended for comprehensive articles. You can close this
                  window and track progress in Automation tab.
                </p>
              </div>
            </div>
            ...
            <Button
              ...
              className="w-full bg-wt-gold hover:bg-wt-gold-hover text-wt-text dark:text-wt-text h-12 text-lg font-semibold"
              size="lg"
            >
              {generating ? (
                <>
            ...
            <div className="flex items-center gap-2 text-xs text-wt-text-muted/70 dark:text-wt-text-muted/70 pt-2 border-t">
              <CheckCircle2 className="w-4 h-4 text-wt-gold" />
              <span>
                Includes: Title, Meta Description, Content, Keywords, Tags, SEO
                Analysis
              </span>
            </div>
          </CardContent>
      ...
      {step === "generating" && (
        <Card>
          <CardContent className="p-6 md:p-8 text-center">
            <Loader2 className="w-12 h-12 animate-spin text-wt-gold mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-wt-text mb-2">
              Generating Your Article
            </h3>
            <p className="text-wt-text-muted/50 dark:text-wt-text-muted/50 mb-4">
            ...
            <div className="space-y-2 max-w-md mx-auto">
              ...
              <div className="flex items-center gap-2 text-sm text-wt-text-muted dark:text-wt-text-muted">
      ...
      {step === "review" && generatedArticle && (
        <div className="space-y-6">
          {/* Success Message */}
          <Card className="bg-wt-gold-subtle border-wt-border-light">
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                <CheckCircle2 className="w-6 h-6 text-wt-gold" />
                <div>
                  <h3 className="font-semibold text-wt-text">
                    Article Generated Successfully!
                  </h3>
                  <p className="text-sm text-wt-gold">
                    Review the article below. You can edit, save as draft, or
                    publish directly.
                  </p>
                </div>
              </div>
          ...
          <Card>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    <Badge variant="outline">
                      {generatedArticle.read_time} min read
                    </Badge>
                    <Badge variant="outline">
                      {generatedArticle.word_count} words
                    </Badge>
                    <Badge className="bg-wt-gold-subtle text-wt-gold border-0">
            ...
            <CardContent className="space-y-6">
              {/* SEO Meta */}
              <div className="bg-wt-surface-hover p-6 md:p-8 rounded-lg space-y-2">
                <div>
                  <Label className="text-xs text-wt-text-muted/50 dark:text-wt-text-muted/50">
                    SEO Title
                  </Label>
                  <p className="text-sm font-semibold text-wt-text">
                    {generatedArticle.seo_title}
                  </p>
                </div>
                <div>
                  <Label className="text-xs text-wt-text-muted/50 dark:text-wt-text-muted/50">
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <Label className="text-xs text-wt-text-muted/50 dark:text-wt-text-muted/50 mb-2 block">
                    Keywords
                  </Label>
              ...
              <div>
                ...
                <div
                  className="prose prose-slate max-w-none bg-white p-4 rounded-lg border border-wt-border max-h-96 overflow-y-auto"
                  dangerouslySetInnerHTML={{
                    __html: DOMPurify.sanitize(
                      generatedArticle.structured_content
              ...
              <div className="flex gap-3 pt-4 border-t">
                ...
                <Button onClick={reset} variant="ghost" disabled={false}>
      ...
      {step === "publishing" && (
        <Card>
          <CardContent className="p-6 md:p-8 text-center">
            ...
            <p className="text-wt-text-muted/50 dark:text-wt-text-muted/50">
              Saving your article...
            </p>
          </CardContent>
        </Card>
