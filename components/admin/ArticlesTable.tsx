...
export default function ArticlesTable({
      ...
      articlesArray.filter((article) => {
        ...
        if (filterStatus === "trash") {
          ...
          const lastUpdated = new Date(
            article.updated_at || article.created_at || 0,
          );
          const now = new Date();
          const ageDays = Math.ceil(
            Math.abs(now.getTime() - lastUpdated.getTime()) /
              (1000 * 60 * 60 * 24),
          );
          const ttlDays =
            CATEGORY_TTLS[article.category || "default"] ||
            CATEGORY_TTLS["default"];
  ...
  const statusCounts = useMemo(() => {
    ...
    articlesArray.forEach((a) => {
      if (a.deleted_at) {
        ...
        const ageDays = Math.ceil(
          Math.abs(Date.now() - lastUpdated.getTime()) / (1000 * 60 * 60 * 24),
        );
        const ttlDays =
          CATEGORY_TTLS[a.category || "default"] || CATEGORY_TTLS["default"];
        if (ageDays > ttlDays) {
          counts.stale++;
        }
      }
  ...
  const handleBulkAction = async (
    action: "publish" | "archive" | "delete" | "restore" | "export",
  ) => {
    if (selectedIds.length === 0) return;

    if (action === "export") {
      const selectedArticles = articlesArray.filter((a) =>
        selectedIds.includes(a.id),
      );
      exportToCSV(
        selectedArticles,
        "articles_export",
        articleCSVColumns as any,
      );
    ...
    try {
      ...
      if (action === "restore" && onBulkRestore)
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-white dark:bg-gray-900 p-4 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
        <div className="relative flex-1 max-w-md w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
          />
        </div>
              ...
              <span
                className={cn(
                  ...
                  status === "stale" &&
                    ...
                    "text-rose-500 opacity-100 font-black",
                )}
              >
                {statusCounts[status]}
              </span>
                ...
                {paginatedArticles.map((article) => {
                  ...
                  if (isEditing) {
                    return (
                      <tr
                        key={article.id}
                        className="bg-emerald-50/50 dark:bg-emerald-900/10 border-2 border-emerald-500/50"
                      >
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-2">
                              ...
                              <input
                                ...
                                className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none"
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {(() => {
                            ...
                            const ageDays = Math.ceil(
                              Math.abs(Date.now() - lastUpdated.getTime()) /
                                (1000 * 60 * 60 * 24),
                            );
                            const ttlDays =
                              CATEGORY_TTLS[article.category || "default"] ||
                            ...
                            if (ageDays > ttlDays) {
                              return (
                                <span
                                  className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 border border-amber-200 dark:border-amber-800"
                                  title={`Stale: ${ageDays} days old (Limit: ${ttlDays})`}
                                >
                        ...
                        {article.category ? (
                          <span
                            className={cn(
                              ...
                              isSelected
                                ? "bg-emerald-200/50 dark:bg-emerald-500/30 text-emerald-900 dark:text-white border-emerald-300 dark:border-emerald-400"
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex flex-col items-end">
                          <span
                            className={cn(
                              "text-xs font-bold",
                              (article.seo_score || 0) >= 80
                                ? "text-emerald-500"
                                : (article.seo_score || 0) >= 50
                                  ? "text-amber-500"
                                  : "text-rose-500",
                            )}
                          >
                      ...
                      <td
                        className={cn(
                          ...
                          isSelected
                            ? "text-emerald-900 dark:text-emerald-50 font-bold"
                            : "text-gray-700 dark:text-gray-200",
                        )}
                      >
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          ...
          >
            <div className="flex items-center gap-3 p-2 px-4 bg-gray-900 dark:bg-gray-800 rounded-full shadow-2xl border-2 border-emerald-500/50 text-white ring-4 ring-black/5">
    ...
    <th
      className={cn(
        ...
        isActive
          ...
          : "hover:bg-gray-100/30 dark:hover:bg-gray-800/20",
