                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-200 dark:border-slate-800">
                                <tr>
                                    ...
                                    <SortableHeader label="Author" field="author_name" sortConfig={sortConfig} onSort={handleSort} />
                                    <SortableHeader label="Category" field="category" sortConfig={sortConfig} onSort={handleSort} />
                                    <SortableHeader label="Views" field="views" sortConfig={sortConfig} onSort={handleSort} align="right" />
                                    <SortableHeader label="Date" field="created_at" sortConfig={sortConfig} onSort={handleSort} align="right" />
                                ...
                                {paginatedArticles.map((article) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className={cn(
                                                "px-4 py-3 text-right font-mono text-xs transition-colors",
                                                isSelected ? "text-sky-900 dark:text-sky-50 font-bold" : "text-slate-700 dark:text-slate-200"
                                            )}>
                                                {article.views?.toLocaleString() || 0}
                                            </td>
                                            <td className={cn(
                                                "px-4 py-3 text-right text-xs transition-colors",
                                                    ...
                                                    {filterStatus === 'trash' ? (
                                                        <>
                                                            ...
                                                            <Button 
                                                                ...
                                                                onClick={() => { if(confirm('Permanently delete this article?')) onDelete?.(article.id) }}
                                                        ...
                                                        <>
                                                            ...
                                                            <Link 
                                                                ...
                                                                target="_blank"
                                                                className="inline-flex items-center justify-center h-8 w-8 rounded-md text-slate-500 hover:text-sky-600 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                                                                title="View"
                                                            >
...
function SortableHeader({ label, field, sortConfig, onSort, align = 'left' }: { label: string, field: string, sortConfig: any, onSort: any, align?: 'left' | 'right' }) {
    const isActive = sortConfig.field === field;
    return (
        <th 
            className={cn(
                "px-4 py-3 font-bold text-slate-700 dark:text-slate-300 cursor-pointer hover:text-sky-600 dark:hover:text-sky-400 transition-colors select-none uppercase tracking-wider text-[11px]",
