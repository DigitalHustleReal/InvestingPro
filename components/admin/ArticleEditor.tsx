...
export default function ArticleEditor({
  ...
  const extensions = React.useMemo(
    () => [
      ...
      Table.configure({
        resizable: true,
        HTMLAttributes: {
          class: "border-collapse border border-gray-700 my-4",
        },
      }),
      TableRow,
      TableHeader,
      TableCell.configure({
        HTMLAttributes: { class: "border border-gray-700 px-4 py-2" },
      }),
  ...
  const editor = useEditor(
      ...
      editorProps: {
        attributes: {
          class: cn(
            ...
            "[&_li]:text-gray-200",
            "[&_strong]:text-white",
            "[&_blockquote]:text-gray-400 [&_blockquote]:border-gray-700",
            "[&_code]:text-gray-100 [&_code]:bg-gray-800 [&_code]:px-1 [&_code]:rounded",
            "[&_a]:text-emerald-400 hover:[&_a]:underline",
            className,
          ),
    ...
    [editable],
  ); // Only recompute if editable changes
  ...
  const generateOutline = useCallback(async () => {
    ...
    editor.commands.setContent(
      originalContent + "<p><em>Generating outline...</em></p>",
    );
  ...
  if (!isReady) {
    return (
      <div className="border border-gray-800 rounded-lg bg-gray-950 overflow-hidden min-h-[400px] flex items-center justify-center">
  ...
  return (
    <div className="bg-gray-950 rounded-xl overflow-hidden shadow-sm border border-gray-800 transition-all duration-300">
      {/* Toolbar */}
      {editable && (
        <div className="border-b border-gray-800 p-2 flex flex-wrap gap-1 bg-gray-900 backdrop-blur-md transition-colors duration-300">
          <Button
            variant="ghost"
            size="sm"
            onClick={() =>
      ...
      {/* Editor Content */}
      <div
        className="p-6 md:p-8 min-h-[500px] bg-gray-950"
        role="textbox"
