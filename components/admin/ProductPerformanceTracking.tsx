 ...
 */
export default function ProductPerformanceTracking({
  timeRange = "30d",
}: ProductPerformanceTrackingProps) {
  // Fetch product performance data
  const {
    data: productStats = { totals: {}, topProducts: [], categoryStats: [] },
  } = useQuery({
  ...
  const {
    data: affiliateStats = { totals: {}, topAffiliates: [], topLinks: [] },
  } = useQuery({
  ...
  const { data: brandStats = { totals: {}, topBrands: [], promotions: [] } } =
    useQuery({
  ...
  const revenueChartData = React.useMemo(() => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      // Zeros until affiliate_clicks aggregation is wired into
      // this admin dashboard. Math.random() removed 2026-04-26.
      data.push({
    ...
    <div className="space-y-10">
        ...
        <Card className="bg-wt-surface dark:bg-wt-surface border-wt-border/50 dark:border-wt-border/50 rounded-xl overflow-hidden hover:border-wt-gold/30 transition-all group">
          <CardContent className="p-8">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[10px] font-bold text-wt-text-muted/70 dark:text-wt-text-muted/70 uppercase tracking-[0.2em]">
                Product Views
              </span>
            ...
            {productStats.topProducts && productStats.topProducts.length > 0 ? (
              productStats.topProducts
                ...
                .map((product: any, idx: number) => (
                  <div
                    key={product.id || idx}
                    className="flex items-center justify-between p-6 hover:bg-wt-surface/50 dark:bg-wt-surface/50 transition-colors group"
                  >
                    <div className="flex items-center gap-5 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <Badge
                            variant="outline"
                            className="text-[9px] border-wt-gold/20 text-wt-gold"
                          >
                    ...
                    <div className="text-right ml-4 space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-[9px] text-wt-text-muted/70 dark:text-wt-text-muted/70">
                        ...
                        <span>•</span>
                        <span className="text-wt-gold">
                          {product.conversionRate?.toFixed(1) || 0}%
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="text-center py-12 text-wt-text-muted/50 dark:text-wt-text-muted/50 font-bold uppercase tracking-widest text-[10px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="bg-wt-surface dark:bg-wt-surface border-wt-border/50 dark:border-wt-border/50 rounded-xl overflow-hidden">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-white/5">
              {affiliateStats.topAffiliates &&
              affiliateStats.topAffiliates.length > 0 ? (
                affiliateStats.topAffiliates
                  .slice(0, 5)
                  .map((affiliate: any, idx: number) => (
                    <div
                      key={affiliate.id || idx}
                      className="flex items-center justify-between p-6 hover:bg-wt-surface/50 dark:bg-wt-surface/50 transition-colors group"
                    >
                      <div className="flex items-center gap-5 flex-1 min-w-0">
                        <div className="w-8 h-8 rounded-lg bg-wt-gold-subtle flex items-center justify-center text-[10px] font-bold text-wt-gold border border-wt-gold/20">
        ...
        <Card className="bg-wt-surface dark:bg-wt-surface border-wt-border/50 dark:border-wt-border/50 rounded-xl overflow-hidden">
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-bold text-wt-text-muted/70 dark:text-wt-text-muted/70 uppercase">
                Conversion Rate
              </span>
            </div>
