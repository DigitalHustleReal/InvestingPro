          ...
          {data.criticalDeadlines.length > 0 && (
            <div className="space-y-2">
              ...
              {data.criticalDeadlines.slice(0, 2).map((item, i) => (
                ...
                >
                  <p className="font-medium text-red-900 line-clamp-1">
                    {item.suggestedTitle}
                  </p>
        ...
        <CardContent className="p-6 bg-gradient-to-br from-wt-surface to-wt-surface-hover border-b border-wt-border">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold text-wt-text mb-2">
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {data.currentTheme.suggestedFocus.map((focus, i) => (
                  <Badge
                    key={i}
                    variant="outline"
                    className="border-wt-border text-wt-text-muted"
                  >
                    {focus.replace(/-/g, " ")}
                  </Badge>
                ))}
            ...
            <div className="text-right">
              ...
              <p className="text-3xl font-bold text-wt-text tabular-nums">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {data.criticalDeadlines.map((item, i) => (
                ...
                >
                  ...
                  <h4 className="font-semibold text-wt-text mb-1">
                    {item.suggestedTitle}
                  </h4>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap gap-1">
                      {item.articleCategories.map((cat, idx) => (
                        <Badge
                          key={idx}
                          variant="outline"
                          className="text-[10px] py-0 h-5 border-wt-border text-wt-text-muted"
                        >
                          {cat.replace(/-/g, " ")}
                        </Badge>
                      ))}
                    ...
                    <Link
                      href={`/admin/articles/new?title=${encodeURIComponent(item.suggestedTitle)}&category=${item.articleCategories[0]}`}
                    >
                      <Button
                        size="sm"
        ...
        <CardContent>
          <div className="grid grid-cols-7 gap-2">
            {data.weeklyPlan.map((day, i) => (
              ...
              >
                <p className="text-xs font-semibold text-wt-text-muted">
                  {day.dayName}
                </p>
                <p className="text-lg font-bold text-wt-text tabular-nums">
                  {day.suggestionsCount}
                </p>
        ...
        <Button
          ...
          className={
            selectedCategory === null
              ? "bg-wt-gold hover:bg-wt-gold-hover text-white border-0"
              : "border-wt-border text-wt-text-muted hover:bg-wt-surface hover:text-wt-text"
          }
        >
          All Categories
        </Button>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className="text-[10px] border-wt-border text-wt-text-muted"
                      >
                        {item.searchIntent}
                      </Badge>
                      {item.articleCategories.map((cat, idx) => (
                        <Badge
                          key={idx}
                          variant="outline"
                          className="text-[10px] border-wt-border text-wt-text-dim"
                        >
                          {cat.replace(/-/g, " ")}
                        </Badge>
                      ))}
                    </div>
                    <h4 className="font-semibold text-wt-text truncate">
                      {item.suggestedTitle}
                    </h4>
        ...
        <CardContent>
          <div className="space-y-3">
            {data.upcomingEvents.slice(0, 8).map((event, i) => (
              <div
                key={i}
                className="flex items-center gap-4 p-3 rounded-r-xl border border-wt-border border-l-4 border-l-wt-gold bg-wt-bg"
              >
                    ...
                    <Badge
                      className={cn(
                        ...
                        event.priority === "critical"
                          ? "bg-wt-danger-subtle text-wt-danger"
                          : event.priority === "high"
                            ? "bg-wt-orange-subtle text-wt-orange"
