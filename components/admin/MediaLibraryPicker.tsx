  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        if (error) {
          // Storage bucket might not exist — return empty
          return [];
        }
        ...
        try {
          ...
          if (error) {
            if (
              ...
              throw new Error(
                `Storage bucket "${STORAGE_BUCKET}" does not exist. Please create it in your Supabase dashboard.`,
              );
            } else if (
              error.message?.includes("new row violates row-level security")
            ) {
              throw new Error(
                "Permission denied. Please check your Supabase Storage bucket policies.",
              );
            } else {
              throw error;
            }
          ...
          toast.error(
            `Upload failed: ${errorMessage}. Please check: 1. Supabase Storage bucket "${STORAGE_BUCKET}" exists 2. Storage policies allow uploads 3. Environment variables are set correctly`,
          );
        } finally {
          setUploading(false);
        }
                ...
                {filteredMedia.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="p-2">
                        <p
                          className="text-xs font-medium text-wt-text truncate"
                          title={item.name}
                        >
                          {item.name}
                        </p>
