...
export function RepurposePanel({
  ...
  const [result, setResult] = useState<{ format: string; data: any } | null>(
    null,
  );
...
function renderRepurposeResult({
  format,
  data,
}: {
  format: string;
  ...
  if (format === "twitter" && data.tweets) {
    return data.tweets.map((t: string, i: number) => (
      <div
        key={i}
        className="mb-4 pb-2 border-b border-wt-border last:border-0"
      >
        ...
        {data.hooks && data.hooks.length > 0 && (
          <div className="mt-4 pt-4 border-t border-wt-border">
            ...
            {data.hooks.map((h: string, i: number) => (
              <div key={i} className="mb-2 italic">
                "{h}"
              </div>
            ))}
          </div>
