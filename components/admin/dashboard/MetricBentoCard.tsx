...
export default function MetricBentoCard({
    ...
    const cardContent = (
        <div className={cn(
            ...
            <div className="mt-4 w-full relative" style={{ height: '80px', minWidth: '0' }}>
                {isMounted && data && data.length > 0 ? (
                    ...
                    <div className="h-[80px] flex items-center gap-2 pt-8">
                        ...
                        {subtext && <span className="text-xs text-muted-foreground/60 font-medium truncate">{subtext}</span>}
                    </div>
                )}
            </div>
            ...
            {data && data.length > 0 && (subtext || trendValue) && (
                <div className="relative z-10 flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                    <span className="text-[10px] text-muted-foreground/60 font-medium">{subtext}</span>
                    {trendValue && (
                        <span className={cn(
                            "text-[10px] font-bold",
    ...
    if (href) {
        return (
            <a href={href} className="block h-full transition-transform active:scale-[0.98]">
                {cardContent}
            </a>
        );
    }
