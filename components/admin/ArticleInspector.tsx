...
export default function ArticleInspector({
  ...
  const [category, setCategory] = useState<ArticleCategory>(
    article.category || "investing-basics",
  );
  const [subCategory, setSubCategory] = useState(
    article.editorial_notes?.sub_category || "",
  );
  ...
  const handleSecondaryKeywordKeyDown = (
    ...
    if (e.key === "Enter" && secondaryKeywordInput.trim()) {
      ...
      if (
        !secondaryKeywords.includes(newKeyword) &&
        newKeyword !== primaryKeyword
      ) {
  ...
  const removeSecondaryKeyword = (keywordToRemove: string) => {
    setSecondaryKeywords(
      secondaryKeywords.filter((k) => k !== keywordToRemove),
    );
  };
  ...
  const validateForPublish = (): boolean => {
    ...
    if (!article.content || article.content.length < 100)
      errors.push("Content is too short (min 100 chars)");
    ...
    if (seoTitle.length > 60)
      toast.warning("SEO Title is longer than 60 characters");
    if (seoDescription.length > 160)
      toast.warning("Meta Description is longer than 160 characters");
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900 border-l border-gray-800 transition-colors duration-300">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto">
        <Tabs defaultValue="metadata" className="w-full">
          <TabsList className="w-full justify-start rounded-none border-b border-gray-800 bg-transparent h-auto p-0">
            <TabsTrigger
              value="metadata"
              className="px-6 py-3 rounded-none border-b-2 border-transparent data-[state=active]:border-emerald-400 text-gray-400 hover:text-white data-[state=active]:text-white font-medium transition-colors"
            >
              Metadata
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="metadata" className="mt-0 pb-10">
            ...
            >
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-gray-900/50 data-[state=open]:bg-gray-900/50 hover:no-underline group transition-colors">
                  <div className="flex items-center gap-2">
                    <Send className="w-4 h-4 text-gray-400 group-hover:text-white group-data-[state=open]:text-primary transition-colors" />
                    <span className="text-sm font-semibold uppercase tracking-wider text-gray-400 group-hover:text-white group-data-[state=open]:text-white transition-colors">
                      Publishing
                    </span>
                  </div>
                </AccordionTrigger>
                <AccordionContent className="px-6 pb-6 pt-2 space-y-4">
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent className="bg-gray-800 border-gray-800 text-gray-200">
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-gray-900/50 data-[state=open]:bg-gray-900/50 hover:no-underline group transition-colors">
                  <div className="flex items-center gap-2">
                    <Search className="w-4 h-4 text-gray-400 group-hover:text-white group-data-[state=open]:text-primary transition-colors" />
                    <span className="text-sm font-semibold uppercase tracking-wider text-gray-400 group-hover:text-white group-data-[state=open]:text-white transition-colors">
                      Classification
                    </span>
                  </div>
                ...
                <AccordionContent className="px-6 pb-6 pt-2 space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-6 text-emerald-400 hover:bg-emerald-900/20 text-[10px]"
                    ...
                    <CategorySelect
                      value={category}
                      onValueChange={(v: string) =>
                        setCategory(v as ArticleCategory)
                      }
                    />
                  </div>
                  <div>
                    ...
                    <SubCategorySelect
                      ...
                      className="mt-1 bg-gray-800 border-gray-800 text-sm"
                    />
                  </div>
                  <div>
                    <Label htmlFor="language" className="text-gray-300 text-xs">
                      Language
                    </Label>
                    <Select
                      value={language}
                      onValueChange={(v: string) =>
                        setLanguage(v as ArticleLanguage)
                      }
                    >
                      <SelectTrigger
                        id="language"
                  ...
                  <div className="pt-2 space-y-4">
                    <div>
                      <Label className="text-gray-300 mb-1.5 block text-xs">
                        Primary Author
                      </Label>
                      <AuthorSelect
                        value={authorId}
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-gray-900/50 data-[state=open]:bg-gray-900/50 hover:no-underline group transition-colors">
                  <div className="flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-gray-400 group-hover:text-white group-data-[state=open]:text-primary transition-colors" />
                    <span className="text-sm font-semibold uppercase tracking-wider text-gray-400 group-hover:text-white group-data-[state=open]:text-white transition-colors">
                      SEO Keywords
                    </span>
                  </div>
                ...
                <AccordionContent className="px-6 pb-6 pt-2 space-y-4">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {secondaryKeywords.map((kw) => (
                          <Badge
                            key={kw}
                            variant="outline"
                            className="text-[11px] bg-gray-800/50"
                          >
                            {kw}
                            <button
                  ...
                  <div>
                    ...
                    <Select
                      value={searchIntent}
                      onValueChange={(v: string) => setSearchIntent(v as any)}
                    >
                      <SelectTrigger
                        id="search-intent"
                        className="bg-gray-800 border-gray-800 text-sm"
                      >
                ...
                <AccordionContent className="px-6 pb-6 pt-2 space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 text-amber-400 hover:bg-amber-900/20 text-[10px]"
                  ...
                  <div>
                    <Label
                      htmlFor="seo-title"
                      className="text-gray-300 text-xs text-xs"
                    >
                      SEO Title
                    </Label>
                    <Input
                      ...
                      maxLength={70}
                      className={cn(
                        "mt-1 bg-gray-800 border-gray-800 text-sm",
                        seoTitle.length > 60 && "border-warning-500",
                      )}
                    />
                    <p
                      className={cn(
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      maxLength={165}
                      className={cn(
                        "resize-none mt-1 bg-gray-800 border-gray-800 text-sm",
                        seoDescription.length > 160 && "border-warning-500",
                      )}
                    />
                    <p
                      className={cn(
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-gray-900/50 data-[state=open]:bg-gray-900/50 hover:no-underline group transition-colors">
                  <div className="flex items-center gap-2">
                    <ImageIcon className="w-4 h-4 text-gray-400 group-hover:text-white group-data-[state=open]:text-primary transition-colors" />
                    <span className="text-sm font-semibold uppercase tracking-wider text-gray-400 group-hover:text-white group-data-[state=open]:text-white transition-colors">
                      Enhancements
                    </span>
                  </div>
                ...
                <AccordionContent className="px-6 pb-6 pt-2 space-y-6">
                  <div className="space-y-3">
                    <Label className="text-gray-300 text-xs text-xs mb-1 block">
                      Featured Media
                    </Label>
                    <FeaturedImageSelector
                      ...
                      keywords={[primaryKeyword, ...secondaryKeywords].filter(
                        Boolean,
                      )}
                    />
                  </div>
                  ...
                  <div className="space-y-3 pt-2">
                    <Label className="text-gray-300 text-xs">Excerpt</Label>
                    <Textarea
                      value={excerpt}
                      ...
                      maxLength={300}
                      className={cn(
                        "resize-none bg-gray-800 border-gray-800 text-sm",
                        excerpt.length > 300 && "border-warning-500",
                      )}
                    />
                    <p
                      className={cn(
              ...
              >
                <AccordionTrigger className="px-6 py-4 hover:bg-gray-900/50 data-[state=open]:bg-gray-900/50 hover:no-underline group transition-colors">
                  <div className="flex items-center gap-2">
                    <Wand2 className="w-4 h-4 text-gray-400 group-hover:text-white group-data-[state=open]:text-primary transition-colors" />
                    <span className="text-sm font-semibold uppercase tracking-wider text-gray-400 group-hover:text-white group-data-[state=open]:text-white transition-colors">
                      Intelligence
                    </span>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Select value={targetLang} onValueChange={setTargetLang}>
                        <SelectTrigger className="flex-1 bg-gray-800 border-gray-800 text-xs">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="hi">Hindi</SelectItem>
