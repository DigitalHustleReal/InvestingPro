 ...
 */
export default function AdvancedMetricsTable({
  timeRange = "30d",
}: {
    ...
    queryFn: async () => {
      try {
        ...
        const response = await fetch(
          `/api/admin/keywords/stats?timeRange=${timeRange}`,
        );
        if (!response.ok)
  ...
  const incomeMetrics = {
    totalRevenue: 2450.0, // Placeholder or fetch from revenue table
    conversions: 85, // Placeholder
    epc: 12.5, // Earnings Per Click
    ...
    <AdminCard noPadding glass>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-4 text-sm">
        ...
        <div className="p-4 border-b md:border-b-0 md:border-r border-border/50">
          ...
          <div className="flex flex-col gap-2">
            <MetricRow
              ...
              value={researchMetrics.keywordsResearched}
            />
            <MetricRow
              label="Active Keywords"
              value={researchMetrics.activeKeywords}
            />
            <MetricRow
              label="Trending Opps"
              value={researchMetrics.trendingKeywords}
              highlight
            />
          </div>
        </div>
        ...
        <div className="p-4 border-b md:border-b-0 xl:border-b 2xl:border-b-0 xl:border-r border-border/50">
          ...
          <div className="flex flex-col gap-2">
            <MetricRow
              ...
              value={publishMetrics.publishedThisPeriod}
            />
        ...
        <div className="p-4 border-b md:border-b-0 md:border-r border-border/50">
          ...
          <div className="flex flex-col gap-2">
            <MetricRow
              label="Total Views"
              value={trackingMetrics.totalViews.toLocaleString()}
            />
            <MetricRow
              label="Avg. Time on Page"
              value={trackingMetrics.avgTimeOnPage}
            />
            <MetricRow
              label="CTR (Click-Through)"
              value={trackingMetrics.ctr}
            />
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="flex flex-col gap-2">
            <MetricRow
              label="Est. Revenue"
              value={`₹${incomeMetrics.totalRevenue?.toLocaleString()}`}
            />
      ...
      <div className="px-6 py-4 bg-muted/20 border-t border-border/50 flex items-start gap-3 text-sm text-muted-foreground">
        ...
        <span className="leading-relaxed">
          <strong className="text-foreground">Insight:</strong> Content
          publishing velocity is up 12% this week. Focus on "Credit Card"
          keywords for higher EPC.
        </span>
      </div>
    </AdminCard>
  );
