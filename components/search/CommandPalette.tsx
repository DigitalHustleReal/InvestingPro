...
export default function CommandPalette({
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const items =
        results.length > 0 ? results : query.length === 0 ? trending : [];
  ...
  const showSuggestions =
    results.length === 0 && query.length < 2 && trending.length === 0;

  const getIcon = (item: SearchResult) => {
    if (item.type === "product")
      return <TrendingUp className="w-5 h-5 text-primary-400" />;
    if (item.type === "tool")
      return <Loader2 className="w-5 h-5 text-accent-400" />;
              ...
              <div className="px-4 pb-4">
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {SUGGESTIONS.map((s) => (
                    ...
                    >
                      <div className="text-xs font-semibold text-gray-300 group-hover/s:text-white transition-colors">
                        {s.label}
                      </div>
            ...
            {displayItems.length > 0 ? (
                ...
                {displayItems.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 py-0.5">
                      ...
                      {item.provider && (
                        <div className="text-[10px] text-primary-400/80 font-medium mt-0.5">
                          {item.provider}
                        </div>
                      )}
                    ...
                    <div
                      className={cn(
                        ...
                        selectedIndex === index &&
                          "translate-x-0 opacity-100 bg-primary-500/20 border-primary-500/30",
                      )}
                    >
                      <ArrowRight
                        className={cn(
              ...
              <div className="py-20 text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Try searching for 'hdfc', 'sip', or 'mutual funds'
                </p>
              </div>
            ) : null}
          </div>
          ...
          <div className="px-6 py-4 bg-black/20 border-t border-white/5 backdrop-blur-md flex items-center justify-between">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2 text-[10px] text-gray-500">
                <kbd className="flex items-center justify-center w-5 h-5 rounded border border-white/10 bg-white/5 text-gray-600">
                  ↑↓
                </kbd>
                <span>Navigate</span>
              </div>
              <div className="flex items-center gap-2 text-[10px] text-gray-500">
                <kbd className="flex items-center justify-center w-8 h-5 rounded border border-white/10 bg-white/5 text-gray-600">
                  ↵
                </kbd>
                <span>Select</span>
              </div>
