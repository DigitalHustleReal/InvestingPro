  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/admin/affiliates?type=contextual&category=${category}&limit=2`,
      );
    ...
    return (
      <div className={cn("space-y-4", className)}>
        ...
        {links.map((link) => (
          <Card
            key={link.id}
            className="bg-white dark:bg-white/[0.03] border-gray-200 dark:border-white/5 hover:border-primary-500/30 transition-all"
          >
