...
export function SWPCalculator() {
  ...
  const calculateSWP = (): SWPResult => {
    ...
    return {
      monthlyWithdrawal: adjustForInflation
        ? currentWithdrawal
        : monthlyWithdrawal,
      totalWithdrawn,
      ...
      <div className="grid lg:grid-cols-2 gap-6">
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <CardDescription>
                  Calculate systematic withdrawal plan with inflation adjustment
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {[30000, 50000, 75000, 100000].map((amount) => (
                    <button
                      ...
                      className={`text-xs px-2 py-0.5 rounded-md font-medium transition-all ${monthlyWithdrawal === amount ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80 text-muted-foreground"}`}
                    >
                      ₹{amount / 1000}K
                    </button>
                  ))}
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {[8, 10, 12, 15].map((rate) => (
                    <button
                      ...
                      className={`text-xs px-2 py-0.5 rounded-md font-medium transition-all ${expectedReturn === rate ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80 text-muted-foreground"}`}
                    >
                      {rate}%
                    </button>
                  ))}
        ...
        >
          ...
          <CardContent className="pt-4 sm:pt-6 relative z-10">
            {result.exhausted && (
              <div className="p-4 bg-accent-50 border border-accent-200 rounded-sm mb-4">
                <div className="flex items-start gap-2">
                  <AlertCircle className="w-4 h-4 text-accent-600 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-sm font-bold text-accent-900 mb-1">
                      Corpus Exhausted
                    </p>
                    <p className="text-xs text-accent-700">
                      Corpus exhausted after {result.monthsExhausted} months.
                      Consider reducing withdrawal or increasing returns.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-4">
              ...
              <div className="text-center p-6 md:p-8 sm:p-4 bg-card rounded-sm shadow-sm border border-border">
                ...
                <p className="text-base sm:text-lg font-extrabold text-foreground">
                  {Math.floor(result.monthsExhausted / 12)}Y{" "}
                  {result.monthsExhausted % 12}M
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                  Your{" "}
                  <span className="font-bold">{formatCurrency(corpus)}</span>{" "}
                  corpus supports{" "}
                  <span className="font-bold text-2xl underline decoration-emerald-400 decoration-2 underline-offset-2">
                    {formatCurrency(monthlyWithdrawal)}/month
                  </span>{" "}
                  for {Math.floor(result.monthsExhausted / 12)} years!
                  {!result.exhausted &&
                    ` With ₹${formatCurrency(result.remainingCorpus)} remaining.`}
                </p>
                <div className="flex flex-wrap gap-2">
                  ...
                  {result.totalWithdrawn > 5000000 && (
                    <Badge
                      ...
                      className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1"
                    >
                      <GraduationCap className="w-3 h-3" /> Comfortable
                      Retirement
                    </Badge>
                  )}
              ...
              <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-bold text-foreground flex items-center gap-2">
                    ...
                    Get Retirement Withdrawal Strategy (Free)
                  </h3>
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Get a personalized retirement withdrawal plan optimized for
                  your {formatCurrency(corpus)} corpus and{" "}
                  {formatCurrency(monthlyWithdrawal)}/month lifestyle.
                </p>
