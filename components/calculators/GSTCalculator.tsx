...
export function GSTCalculator() {
  ...
  const allRatesData = [5, 12, 18, 28].map((rate) => {
    const base =
      calculationType === "exclusive" ? amount : (amount * 100) / (100 + rate);
    const gst =
      calculationType === "exclusive" ? (amount * rate) / 100 : amount - base;
              ...
              <div className="grid grid-cols-3 gap-2 p-3 bg-muted rounded-lg mb-4">
                <div className="text-center">
                  ...
                  <p className="text-sm font-bold text-foreground">
                    {formatCurrencyShort(result.baseAmount)}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-muted-foreground mb-1">GST</p>
                  <p className="text-sm font-bold text-foreground">
                    {formatCurrencyShort(result.gstAmount)}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-muted-foreground mb-1">Total</p>
                  <p className="text-sm font-bold text-foreground">
                    {formatCurrencyShort(result.totalAmount)}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: string) => setGstRate(Number(value))}
                  >
                    <SelectTrigger className="h-10">
                      <SelectValue />
                    </SelectTrigger>
            ...
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                <CardTitle className="text-xl mb-1 flex items-center gap-2">
                  <Receipt className="w-5 h-5 text-primary-600" /> GST
                  Calculator
                </CardTitle>
                <CardDescription>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <ul className="space-y-1 text-xs text-muted-foreground">
                    <li>
                      • <strong>CGST:</strong> Central GST (half of GST rate)
                    </li>
                    <li>
                      • <strong>SGST:</strong> State GST (half of GST rate)
                    </li>
                    <li>
                      • <strong>IGST:</strong> Integrated GST (inter-state
                      transactions)
                    </li>
                  </ul>
                </div>
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        width: `${(result.baseAmount / result.totalAmount) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        width: `${(result.gstAmount / result.totalAmount) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                    ...
                    formatter={
                      ((
                        ...
                        value !== undefined
                          ...
                          : "",
                        name === "total" ? "Total Amount" : "GST Amount",
                      ]) as never
                    }
                  />
                  <Bar
                  ...
                  >
                    {allRatesData.map((entry, index) => (
                      <Cell
                        key={`cell-gst-${index}`}
                        fill={
                          entry.name === `${gstRate}%` ? "#166534" : "#16a34a"
                        }
                      />
                    ))}
                  </Bar>
            ...
            <div className="space-y-4">
                    ...
                    <table className="w-full min-w-full">
                      ...
                      <tbody className="divide-y divide-border">
                        {allRatesTable.map((row, idx) => (
                          <tr
                            key={idx}
                            className={`hover:bg-muted/50 transition-colors ${row.rate === gstRate ? "bg-primary/5" : ""}`}
                          >
                            <td className="px-3 py-2.5 text-sm font-semibold text-foreground flex items-center gap-1">
                              {row.rate}%
                              {row.rate === gstRate && (
                                <Badge className="ml-1 text-[9px] px-1 py-0 bg-primary/20 text-primary border-0">
                                  Active
                                </Badge>
                              )}
                            </td>
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                  Your {formatCurrency(result.baseAmount)} purchase has{" "}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    <CheckCircle2 className="w-3 h-3" /> CGST:{" "}
                    {formatCurrency(result.cgst)}
                  </Badge>
                  <Badge
                    variant="outline"
                    className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1"
                  >
                    <CheckCircle2 className="w-3 h-3" /> SGST:{" "}
                    {formatCurrency(result.sgst)}
                  </Badge>
                </div>
              </div>
              ...
              <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Get GST compliance tips, invoice templates, and a guide to GST
                  filing for your business — completely free.
                </p>
