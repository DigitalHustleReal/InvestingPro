...
export function LumpsumCalculatorWithInflation() {
  ...
  const calculateLumpsum = () => {
    const futureValue =
      lumpsum * Math.pow(1 + lumpsumReturn / 100, lumpsumYears);
  ...
  const generateYearlyData = () => {
    ...
    for (let year = 0; year <= lumpsumYears; year++) {
      ...
      const realValue = adjustForInflation
        ? futureValue / Math.pow(1 + inflationRate / 100, year)
        : futureValue;
  ...
  const displayValue = adjustForInflation
    ? lumpsumResult.realValue
  ...
  const inputSliders = (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Slider
          value={[lumpsumReturn]}
          onValueChange={(v) => setLumpsumReturn(v[0])}
          min={1}
          max={30}
          step={0.5}
          className="py-2"
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <CardDescription>
                  Calculate returns on one-time investment with inflation
                  adjustment
                </CardDescription>
              </div>
            ...
            <div className="space-y-4">
                      ...
                      <tbody className="divide-y divide-border">
                        ...
                        {!showAllYears && yearlyData.length > 10 && (
                          ...
                          >
                            <td
                              colSpan={4}
                              className="px-3 py-3 text-xs text-center text-primary font-bold"
                            >
                        ...
                        {yearlyData.length > 0 && (
                          <tr className="bg-primary/10 border-t-2 border-primary/20">
                            ...
                            <td className="px-3 py-3 text-sm text-right font-bold text-foreground">
                              {multiplier}x
                            </td>
                          </tr>
                        )}
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                  Your{" "}
                  <span className="font-bold">{formatCurrency(lumpsum)}</span>{" "}
                  one-time investment grows to{" "}
                  <span className="font-bold text-2xl underline decoration-emerald-400 decoration-2 underline-offset-2">
                    {formatCurrency(displayValue)}
                  </span>{" "}
                  in {lumpsumYears} years —{" "}
                  <span className="font-bold">{multiplier}x</span> your money!
                </p>
              ...
              <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Get top mutual fund recommendations for lumpsum investment
                  targeting {formatCurrency(displayValue)}.
                </p>
