  ...
  const results = useMemo(() => {
    ...
    for (let y = 1; y <= tenure; y++) {
      ...
      const loanBalance =
        ...
        emi * ((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate);
      const sipCorpus =
        emi *
    ...
    const chartData = [
      {
        year: "Y0",
        sipCorpus: 0,
        homeEquity: homePrice * (downPaymentPct / 100),
      },
    ];
    for (let y = 1; y <= tenure; y++) {
      const months = y * 12;
  ...
  const InputSliders = () => (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Slider
          value={[sipReturn]}
          onValueChange={(v) => setSipReturn(v[0])}
          min={10}
          max={20}
          step={0.5}
          className="py-2"
            ...
            <CardContent className="space-y-4 pt-0">
              <div className="grid grid-cols-3 gap-2 p-3 bg-muted rounded-lg mb-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm font-bold text-foreground">
                    {formatCurrency(results.sipWealth)}
                  </p>
                </div>
              </div>
              <InputSliders />
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <CardDescription>
                  Compare buying a home vs investing the EMI in mutual funds
                </CardDescription>
              </div>
            ...
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
              ...
              {[
                ...
                <button
                  key={idx}
                  onClick={() => {
                    setHomePrice(p.price);
                    setDownPaymentPct(p.dp);
          ...
          <CardContent className="pt-4 sm:pt-6 relative z-10">
            ...
            <div className="p-4 bg-card rounded-sm border border-border mb-4">
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">
                Comparison After {tenure} Years
              </p>
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-medium text-foreground flex items-center gap-1">
                      <Home className="w-3 h-3" /> Home Equity
                    </span>
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        width: `${Math.min(100, (homePrice / Math.max(homePrice, results.sipWealth)) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="font-medium text-foreground flex items-center gap-1">
                      <TrendingUp className="w-3 h-3" /> SIP Wealth
                    </span>
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        ...
                        width: `${Math.min(100, (results.sipWealth / Math.max(homePrice, results.sipWealth)) * 100)}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="p-3 bg-card rounded-sm border border-border">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {downPaymentPct}% down payment of{" "}
                {formatCurrency((homePrice * downPaymentPct) / 100)}
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-4">
                      ...
                      <tbody className="divide-y divide-border">
                        ...
                        {!showAllYears && results.yearlyData.length > 10 && (
                          ...
                          >
                            ...
                            >
                              Show {results.yearlyData.length - 10} more
                              years...
                            </td>
                          </tr>
                        )}
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                  After {tenure} years: Home loan costs{" "}
                  <span className="font-bold">
                    {formatCurrency(results.totalInterest)}
                  </span>{" "}
                  in interest, SIP would have grown to{" "}
                  <span className="font-bold">
                    {formatCurrency(results.sipWealth)}
                  </span>
                  .{" "}
                  {sipWins
                    ? "SIP creates more wealth!"
                    : "Home ownership has equity advantage."}
                </p>
                <div className="flex flex-wrap gap-2">
                  {sipWins && (
                    ...
                    >
                      <TrendingUp className="w-3 h-3" /> SIP Wins by{" "}
                      {formatCurrency(results.sipWealth - homePrice)}
                    </Badge>
                  )}
                  <Badge
                    variant="outline"
                  ...
                  <Badge
                    ...
                    className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1"
                  >
                    <CheckCircle2 className="w-3 h-3" /> EMI:{" "}
              ...
              <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Get a personalized home loan vs investment analysis with top
                  fund recommendations and EMI optimization strategies.
                </p>
