...
export function GoalPlanningCalculator() {
    ...
    const generateGrowthData = () => {
        ...
        const futureGoal = adjustForInflation
            ? goalAmount * Math.pow(1 + inflationRate / 100, years)
            : goalAmount;
        const requiredSIP = result.requiredSIP;
    ...
    const generateYearlyBreakdown = () => {
        ...
        for (let year = 1; year <= years; year++) {
            ...
            const sipValue = requiredSIP * ((Math.pow(1 + monthlyRate, year * 12) - 1) / monthlyRate) * (1 + monthlyRate);
            corpus = corpus + sipValue;
            const totalInvestedSoFar = currentSavings + requiredSIP * year * 12;
            const progress = Math.min(100, (corpus / futureGoal) * 100);
            data.push({
                year,
    ...
    const totalInvested = currentSavings + result.requiredSIP * years * 12;
    const totalReturns = result.futureGoal - totalInvested;

    const pieData = [
        { name: 'Total Invested', value: Math.max(0, Math.round(totalInvested)), color: '#166534' },
        { name: 'Returns', value: Math.max(0, Math.round(totalReturns)), color: '#16a34a' },
    ];
    ...
    const InputSliders = () => (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <div className="flex justify-between items-center">
                    <Label className="text-sm text-foreground font-semibold">Years to Goal</Label>
                    <div className="flex items-center gap-1.5 bg-muted rounded-lg px-3 py-1.5">
            ...
            <div className="space-y-2">
                ...
                <Slider value={[expectedReturn]} onValueChange={(v) => setExpectedReturn(v[0])} min={8} max={20} step={0.5} className="py-2" />
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-muted/50 rounded-sm border border-border">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground">Project future goal with inflation</p>
                    </div>
                </div>
                    ...
                    <CardHeader className="cursor-pointer" onClick={() => setInputsExpanded(!inputsExpanded)}>
                        <div className="flex items-center justify-between">
                            <div className="flex-1">
                                <CardTitle className="text-lg mb-1">Goal Planning</CardTitle>
                        ...
                        <CardContent className="space-y-4 pt-0">
                            <div className="grid grid-cols-3 gap-2 p-3 bg-muted rounded-lg mb-4">
                                <div className="text-center">
                                    ...
                                    <p className="text-sm font-bold text-foreground">{formatCurrency(goalAmount)}</p>
                                </div>
                                <div className="text-center">
                                ...
                                <div className="text-center">
                                    <p className="text-xs text-muted-foreground mb-1">SIP/mo</p>
                                    <p className="text-sm font-bold text-foreground">{formatCurrency(result.requiredSIP)}</p>
                                </div>
                            </div>
                            <InputSliders />
                        </CardContent>
                ...
                <Card className="hidden lg:block border-border shadow-sm rounded-sm">
                    <CardHeader>
                        <div className="flex items-start justify-between gap-4 mb-2">
                            <div className="flex-1">
                                <CardTitle className="text-xl mb-1">Goal Planning Calculator</CardTitle>
                                <CardDescription>Calculate the monthly SIP needed to achieve your financial goals</CardDescription>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                            ...
                            {[
                                { label: "Dream Home ₹50L 10yr", goal: 5000000, yrs: 10, ret: 12 },
                                { label: "Child Education ₹20L 8yr", goal: 2000000, yrs: 8, ret: 12 },
                                { label: "Car ₹8L 3yr", goal: 800000, yrs: 3, ret: 10 },
                                { label: "Retirement ₹2Cr 25yr", goal: 20000000, yrs: 25, ret: 12 },
                            ].map((p, idx) => (
                                <button key={idx}
                                    onClick={() => { setGoalAmount(p.goal); setYears(p.yrs); setExpectedReturn(p.ret); }}
                    ...
                    <CardContent className="pt-4 sm:pt-6 relative z-10">
                        ...
                        <div className="flex justify-center gap-6 -mt-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-medium text-muted-foreground">Invested</span>
                            </div>
                            <div className="flex items-center gap-2">
                ...
                <Card className="border-border shadow-sm rounded-sm">
                    ...
                    <CardContent>
                        <div className="h-80 lg:h-96">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={growthData}>
                                    <defs>
                                        <linearGradient id="colorSIPGoalValue" x1="0" y1="0" x2="0" y2="1">
                                            <stop offset="5%" stopColor="#166534" stopOpacity={0.3} />
                                            <stop offset="95%" stopColor="#166534" stopOpacity={0} />
                                        </linearGradient>
                                        <linearGradient id="colorSIPGoalTarget" x1="0" y1="0" x2="0" y2="1">
                                    ...
                                    />
                                    <Area type="monotone" dataKey="corpus" stroke="#166534" fill="url(#colorSIPGoalValue)" strokeWidth={2} name="Your Corpus" />
                                    <Area type="monotone" dataKey="goal" stroke="#d97706" fill="url(#colorSIPGoalTarget)" strokeWidth={2} strokeDasharray="5 5" name="Goal Amount" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                        <div className="flex justify-center gap-6 mt-2">
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#166534' }} />
                                <span className="text-xs font-medium text-muted-foreground">Your Corpus</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#d97706' }} />
                                <span className="text-xs font-medium text-muted-foreground">Goal Amount</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-4 bg-primary-50 rounded-sm border border-primary-100">
                                    ...
                                    <p className="text-lg font-bold text-foreground">{formatCurrency(result.requiredSIP)}</p>
                                </div>
                                <div className="p-4 bg-primary-50 rounded-sm border border-primary-100">
                                            ...
                                            <tbody className="divide-y divide-border">
                                                {(showAllYears ? yearlyData : yearlyData.slice(0, 10)).map((row, idx) => (
                                                    <tr key={idx} className="hover:bg-muted/50 transition-colors">
                                                        ...
                                                        <td className="px-3 py-2.5 text-sm text-right font-medium text-muted-foreground">{formatCurrency(row.corpus)}</td>
                                                    </tr>
                                                ))}
                                                ...
                                                <tr className="bg-primary/10 border-t-2 border-primary/20">
                                                    ...
                                                    <td className="px-3 py-3 text-sm text-right font-bold text-primary">{formatCurrency(totalInvested)}</td>
                                                    <td className="px-3 py-3 text-sm text-right font-bold text-primary">{formatCurrency(result.futureGoal)}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                            ...
                            <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                                ...
                                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                                    Invest <span className="font-bold">{formatCurrency(result.requiredSIP)}/month</span> to reach your <span className="font-bold">{formatCurrency(goalAmount)}</span> goal in {years} years.
                                </p>
                                <div className="flex flex-wrap gap-2">
                                    {goalAmount >= 20000000 && (
                                        <Badge variant="outline" className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1">
                                            <TrendingUp className="w-3 h-3" /> Retirement Planning
                                        </Badge>
                                    )}
                                    {goalAmount >= 5000000 && goalAmount < 20000000 && (
                                        <Badge variant="outline" className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1">
                                            <Home className="w-3 h-3" /> Dream Home Goal
                                        </Badge>
                                    )}
                                    {goalAmount >= 2000000 && goalAmount < 5000000 && (
                                        <Badge variant="outline" className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1">
                                            <GraduationCap className="w-3 h-3" /> Education Fund
                                        </Badge>
                                    )}
                                    {goalAmount < 2000000 && goalAmount >= 800000 && (
                                        <Badge variant="outline" className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1">
                                    ...
                                    <Badge variant="outline" className="bg-white/50 border-emerald-200 text-emerald-800 flex items-center gap-1">
                                        <CheckCircle2 className="w-3 h-3" /> {adjustForInflation ? 'Inflation Adjusted' : 'Nominal Goal'}
                                    </Badge>
                                </div>
                            </div>
                            ...
                            <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                                ...
                                <p className="text-xs text-muted-foreground mb-4">Get a personalized investment plan with top fund recommendations to achieve your {formatCurrency(goalAmount)} goal in {years} years.</p>
