...
export function CompoundInterestCalculator() {
    ...
    const yearlyData = Array.from({ length: time }, (_, i) => {
        ...
        const prevVal = yr === 1 ? principal : principal * Math.pow(1 + r / n, n * (yr - 1));
        return {
            year: yr,
    ...
    const growthData = [
        { year: 'Y0', value: Math.round(principal), siValue: Math.round(principal) },
    ...
    const frequencyLabel: Record<string, string> = { '1': 'Annual', '2': 'Semi-Annual', '4': 'Quarterly', '12': 'Monthly', '365': 'Daily' };
                    ...
                    <CardHeader className="cursor-pointer" onClick={() => setInputsExpanded(!inputsExpanded)}>
                        <div className="flex items-center justify-between">
                            <div className="flex-1">
                                <CardTitle className="text-lg mb-1">Compound Interest Calculator</CardTitle>
                ...
                <Card className="hidden lg:block border-border shadow-sm rounded-xl">
                    <CardHeader>
                        <div className="flex items-start justify-between gap-4 mb-2">
                            <div className="flex-1">
                                <CardTitle className="text-xl mb-1">Compound Interest Calculator</CardTitle>
                                <CardDescription>See the power of compounding on your investment</CardDescription>
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                            ...
                            {presets.map((preset, idx) => (
                                <button key={idx} onClick={() => { setPrincipal(preset.p); setRate(preset.r); setTime(preset.t); setFrequency(preset.f); }}
                ...
                <Card id="calculator-results" className="order-first lg:order-none border-border shadow-sm rounded-xl bg-gradient-to-br from-primary/5 to-secondary/5 relative overflow-hidden">
                    ...
                    <CardContent className="pt-4 sm:pt-6 relative z-10">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-4">
                            ...
                            <div className="text-center p-6 md:p-8 sm:p-4 bg-card rounded-xl shadow-sm border border-border">
                                <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5 sm:mb-2">CI RETURNS</p>
                        ...
                        <div className="flex justify-center mb-3">
                            <span className="inline-flex items-center gap-1.5 text-xs bg-muted px-3 py-1.5 rounded-full border border-border text-muted-foreground">
                                ...
                                {frequencyLabel[frequency]} compounding · {multiplier.toFixed(2)}x growth
                            </span>
                        </div>
                        {/* Pie chart */}
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                ...
                <Card className="border-border shadow-sm rounded-xl">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-bold uppercase tracking-widest text-muted-foreground">CI vs Simple Interest Growth</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="h-80 lg:h-96">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart data={growthData}>
                                    <defs>
                                        <linearGradient id="colorCIValue" x1="0" y1="0" x2="0" y2="1">
                                            <stop offset="5%" stopColor="#166534" stopOpacity={0.3} />
                                            <stop offset="95%" stopColor="#166534" stopOpacity={0} />
                                        </linearGradient>
                                        <linearGradient id="colorSICompare" x1="0" y1="0" x2="0" y2="1">
                                    ...
                                    <Area type="monotone" dataKey="value" stroke="#166534" fill="url(#colorCIValue)" strokeWidth={2} name="Compound Interest" />
                                    <Area type="monotone" dataKey="siValue" stroke="#d97706" fill="url(#colorSICompare)" strokeWidth={2} strokeDasharray="5 5" name="Simple Interest" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                        <div className="flex gap-4 mt-3 justify-center">
                            ...
                            <div className="flex items-center gap-1.5">
                                <div className="w-4 h-0.5 bg-[#d97706]" style={{ borderTop: '2px dashed #d97706', height: 0 }} />
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="p-4 bg-primary-50 rounded-xl border border-primary-100">
                                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">CI ADVANTAGE</p>
                                    <p className="text-lg font-bold text-foreground">+{formatCurrency(Math.round(ciAdvantage))}</p>
                                </div>
                                <div className="p-4 bg-primary-50 rounded-xl border border-primary-100">
                                    <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">GROWTH MULTIPLE</p>
                                    <p className="text-lg font-bold text-foreground">{multiplier.toFixed(2)}x</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-xl p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                                ...
                                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                                    Your {formatCurrency(principal)} grows to {formatCurrency(Math.round(totalAmount))} — {multiplier.toFixed(2)}x your investment in {time} years. CI earns {formatCurrency(Math.round(ciAdvantage))} more than simple interest!
                                </p>
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <span className="inline-flex items-center gap-1 text-xs bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 px-2.5 py-1 rounded-full font-medium">
                                        <TrendingUp className="w-3 h-3" /> {((totalInterest / principal) * 100).toFixed(1)}% Return
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-5 bg-card rounded-xl border border-border shadow-sm mt-4">
                                ...
                                <p className="text-xs text-muted-foreground mb-4">Receive a detailed compound interest analysis with CI vs SI comparison, year-by-year schedule, and wealth-building tips.</p>
