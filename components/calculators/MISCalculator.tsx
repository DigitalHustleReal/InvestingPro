...
export function MISCalculator() {
  ...
  const chartData = [
    {
      name: "Principal",
      value: Math.round(effectiveInvestment),
      color: "#166534",
    },
    {
      name: "Total Interest",
  ...
  const inputSliders = (
    <div className="space-y-4">
      {/* Account Type Toggle */}
      <div className="flex items-center gap-2 p-3 bg-muted/50 rounded-sm border border-border">
        <button
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-1.5 bg-muted rounded-lg px-3 py-1.5">
            ...
            <Input
              ...
              value={investment}
              onChange={(e) =>
                setInvestment(Math.min(Number(e.target.value), maxInvestment))
              }
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          ...
          <span className="bg-indian-gold/20 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium">
            Current: 7.4% p.a. monthly payout
          </span>
          <span>9%</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 bg-muted/50 rounded-sm border border-border text-center">
      ...
      <div className="p-4 bg-muted/50 rounded-sm border border-border flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            No premature withdrawal before 1 year. Principal fully returned at
            maturity (5 years). Available at all Post Offices.
          </p>
        </div>
      </div>
    </div>
          ...
          <CardHeader>
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex-1">
                ...
                <CardDescription>
                  Post Office Monthly Income Scheme — Guaranteed monthly payout
                </CardDescription>
              </div>
          ...
          <CardContent className="pt-4 sm:pt-6 relative z-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-4">
              ...
              <div className="text-center p-6 md:p-8 sm:p-4 bg-card rounded-sm shadow-sm border border-border">
                ...
                <p className="text-base sm:text-lg font-extrabold text-primary">
                  {formatCurrency(Math.round(annualInterest))}
                </p>
              </div>
              <div className="text-center p-6 md:p-8 sm:p-4 bg-card rounded-sm shadow-sm border border-border">
                <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5 sm:mb-2">
            ...
            <div className="flex justify-center mb-3">
              <span className="inline-flex items-center gap-1.5 text-xs bg-muted px-3 py-1.5 rounded-full border border-border text-muted-foreground">
                ...
                {isJointAccount ? "Joint Account" : "Single Account"} ·{" "}
                {interestRate}% p.a. · Monthly payout
              </span>
            </div>
            {/* Pie chart */}
            ...
            <div className="space-y-4">
                      ...
                      <tbody className="divide-y divide-border">
                        ...
                        <tr className="bg-primary/10 border-t-2 border-primary/20">
                          ...
                          <td className="px-3 py-3 text-sm text-right font-bold text-primary">
                            {formatCurrency(Math.round(annualInterest))}/yr
                          </td>
                          <td className="px-3 py-3 text-sm text-right font-bold text-primary">
                            {formatCurrency(Math.round(totalInterest))}
                          </td>
              ...
              <div className="bg-gradient-to-br from-emerald-50 to-emerald-50 dark:from-emerald-950/30 dark:to-emerald-950/30 rounded-sm p-5 border border-emerald-100 dark:border-emerald-900/50 relative overflow-hidden">
                ...
                <p className="text-lg font-medium text-emerald-900 dark:text-emerald-100 mb-3">
                  Your MIS deposit earns{" "}
                  {formatCurrency(Math.round(monthlyInterest))} every month — a
                  guaranteed income of{" "}
                  {formatCurrency(Math.round(monthlyInterest))}/month for 5
                  years from {formatCurrency(effectiveInvestment)}.
                </p>
                <div className="flex flex-wrap gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 text-xs bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 px-2.5 py-1 rounded-full font-medium">
                    <TrendingUp className="w-3 h-3" /> Principal Returned at
                    Maturity
                  </span>
                </div>
              </div>
              ...
              <div className="p-5 bg-card rounded-sm border border-border shadow-sm mt-4">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Get a complete MIS income schedule with month-wise payout
                  tracker, comparison with FD/RD, and passive income planning
                  tips.
                </p>
