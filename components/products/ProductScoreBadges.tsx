...
export function calculateProductTags(product: any, category: string): string[] {
  ...
  if (category === "credit_card") {
    ...
    if (
      product.loungeAccess &&
      !product.loungeAccess.toLowerCase().includes("nil")
    ) {
      tags.push("Best for Travel");
    }
    // Shopping benefit detection
    if (
      product.type?.toLowerCase().includes("shopping") ||
  ...
  if (category === "loan") {
    ...
    const rate =
      product.interestRateMin || parseFloat(product.interest_rate || "15");
    if (rate < 10) {
    ...
    // Loan type
    if (
      product.loanType === "home" ||
      product.type?.toLowerCase().includes("home")
    ) {
