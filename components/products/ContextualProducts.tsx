...
export default function ContextualProducts({ category }: { category: string }) {
    ...
    if (isLoading) {
        return (
            <div className="my-16 p-8 bg-gray-900 rounded-xl text-white border border-gray-700 shadow-2xl relative overflow-hidden">
    ...
    return (
        <div className="my-16 p-8 bg-gray-900 rounded-xl text-white border border-gray-700 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <p className="text-gray-600 mb-8 max-w-lg text-sm">Based on the analysis in this article, these are currently the best {category.replace('-', ' ')} options in India for 2026.</p>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {Array.isArray(products) && products.map(p => (
                        <Card key={p.id} className="bg-gray-800/40 border-gray-700/50 p-6 flex flex-col hover:border-primary-500/50 transition-all group backdrop-blur-md">
                            <div className="flex justify-between items-start mb-5">
                                ...
                                <div className="text-accent-400 flex items-center gap-1.5 text-xs font-bold bg-gray-900/80 px-2.5 py-1 rounded-full border border-gray-700">
                            ...
                            <p className="text-[12px] text-gray-600 mb-6 line-clamp-2 leading-relaxed opacity-80">{p.description}</p>
                            ...
                            <div className="mt-auto">
                                ...
                                <p className="text-[9px] text-center text-gray-500 mt-3 uppercase tracking-widest font-bold">Secure Partner Link</p>
                            </div>
                        </Card>
                    ))}
                </div>
