...
export function RichProductCard({ product, layout = 'grid', onCompare, matchScore, isScored, scoreBreakdown, rawScore }: RichProductCardProps) {
    ...
    const getTrustColor = (score: number) => {
        if (!score) return 'text-gray-500 ring-gray-500/20';
    ...
    return (
        <Card className={cn(
            "group relative overflow-hidden border-gray-200 transition-all hover:border-primary-500/50 hover:shadow-xl hover:shadow-primary-500/5",
            ...
            {(isScored && matchScore !== undefined) && (
                <div className="absolute top-10 right-0 z-10 animate-in slide-in-from-right-4 duration-500">
                     <ScoreExplanation overall={rawScore || (matchScore / 10)} breakdown={scoreBreakdown || []}>
                        <div className={cn(
                            "bg-white/90 backdrop-blur border border-gray-200 text-gray-900 text-[10px] font-black px-2 py-1 rounded-l-lg shadow-sm flex items-center gap-1 cursor-help group/badge",
                            matchScore > 80 ? "border-success-200" : "border-gray-200"
                        )}>
                            <div className={cn(
                                "w-1.5 h-1.5 rounded-full animate-pulse", 
                                matchScore > 80 ? "bg-success-500" : (matchScore > 60 ? "bg-amber-500" : "bg-gray-400")
                            )} />
                            {matchScore}% Match
                        </div>
                     </ScoreExplanation>
                ...
                <button 
                    ...
                    className={cn(
                        ...
                        isCompareSelected 
                            ...
                            : "bg-white/90 backdrop-blur-md border-gray-200 text-gray-600 hover:border-primary-500 hover:text-primary-500"
                    )}
                    aria-label={isCompareSelected ? "Remove from comparison" : "Add to comparison"}
                >
                    <div className={cn(
                        "w-5 h-5 rounded-md border flex items-center justify-center transition-colors",
                        isCompareSelected ? "bg-white border-white" : "border-gray-300"
                    )}>
            ...
            <div className={cn(
                ...
                isList ? "w-full md:w-80 border-b md:border-b-0 md:border-r border-gray-100" : "border-b border-gray-100"
            )}>
                ...
                <div className="w-full max-w-[280px] aspect-[1.6/1] rounded-xl overflow-hidden mb-4 shadow-lg hover:shadow-xl transition-shadow relative">
                    {product.image_url ? (
                        ...
                        <div className="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center rounded-xl">
                            <span className="text-5xl font-bold text-gray-600">{product.name.charAt(0)}</span>
                        </div>
                    )}
                </div>
            ...
            <div className="flex-1 flex flex-col">
                <CardHeader className="pb-2">
                    <div className="flex justify-between items-start gap-4">
                        <div className="flex-1">
                            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">{product.provider_name}</p>
                            <h3 className="text-lg font-bold text-gray-900 leading-tight group-hover:text-primary-700 transition-colors mb-2">
                                {product.name}
                            </h3>
                        ...
                        <div className="flex flex-col items-end">
                            <div className="flex items-center bg-accent-50 px-2 py-1 rounded-lg border border-accent-100">
                                ...
                                <span className="font-bold text-gray-900 text-sm">{typeof ratingObj.overall === 'number' ? ratingObj.overall.toFixed(1) : ratingObj.overall}</span>
                            </div>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                        {product.key_features.slice(0, 4).map((feat, i) => (
                            <div key={i} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                                <p className="text-[10px] text-gray-500 mb-0.5">{feat.label}</p>
                                <p className="font-semibold text-gray-900 text-sm truncate">{feat.value}</p>
                            </div>
                        ))}
                    </div>

                    {/* Trust Score Indicator & Fee Calculator */}
                    <div className="flex items-center justify-between bg-gray-50/50 rounded-lg p-2 mb-4 border border-gray-100">
                        ...
                        <div className="group/calc relative">
                            ...
                            <div className="absolute bottom-full right-0 mb-2 w-48 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-200 dark:border-gray-800 p-3 opacity-0 group-hover/calc:opacity-100 transition-opacity pointer-events-none z-20">
                                <p className="text-[10px] font-semibold text-gray-500 mb-2 uppercase">Net Value Calculator</p>
                                <div className="space-y-1">
                                    ...
                                    <div className="border-t border-gray-100 my-1 pt-1 flex justify-between text-xs font-bold">
                                        <span>Net Gain:</span>
                ...
                <CardFooter className="pt-0 gap-3 border-t border-gray-50 p-4 bg-gray-50/30 flex-col items-stretch">
