...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const rewardRate =
    product.features?.["reward_rate"] ||
    product.features?.["intraday_brokerage"];
    ...
    <Card className="flex flex-col h-full hover:shadow-xl transition-shadow relative overflow-hidden group border-gray-200">
      ...
      <div className="absolute top-3 right-3 z-10 bg-white/95 p-1.5 rounded-lg shadow-sm backdrop-blur-sm border border-gray-100 flex items-center gap-2">
        <Checkbox
          ...
          checked={selected}
          onCheckedChange={(c) =>
            c ? addProduct(richProduct) : removeProduct(product.id)
          }
      ...
      <CardFooter className="p-6 md:p-8 pt-0 gap-3 flex-col">
        <div className="flex gap-3 w-full">
          ...
          <TrackedAffiliateLink
            ...
            sourcePage={product.category?.replace(/_/g, "-") || "products"}
            sourceComponent="product_card"
            variant="button"
            showIcon={false}
            className="flex-1 w-full bg-primary-dark hover:bg-primary-800 text-white font-semibold shadow-md hover:shadow-lg transition-all h-10 rounded-md text-sm"
          >
            Apply Now
          </TrackedAffiliateLink>
        </div>
        ...
        <div className="w-full pt-2 border-t border-gray-100 flex items-center justify-between">
          ...
          <Link
            ...
            className="text-[10px] text-gray-600 hover:text-primary-600 transition-colors"
          >
            How we rate
          </Link>
        </div>
