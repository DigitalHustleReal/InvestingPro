...
export default function DifferentiationCard({ score, productName }: DifferentiationCardProps) {
    ...
    return (
        <Card className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 text-gray-900 dark:text-white overflow-hidden border-gray-200 dark:border-gray-700">
            ...
            <CardContent>
                ...
                <div className="flex justify-between items-start mb-6">
                    <div>
                         <div className="flex flex-wrap gap-2 mb-2">
                            {score.tags.map(tag => (
                                <span key={tag} className="text-xs font-bold bg-gray-900/5 dark:bg-white/10 text-gray-700 dark:text-white px-2 py-1 rounded-full border border-gray-200 dark:border-white/10">
                                    {tag}
                                </span>
                            ))}
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-600 max-w-[200px]">
                            Calculated based on rewards, fees, and benefits analysis.
                        </p>
                    </div>
                    
                    <div className="text-center bg-white/50 dark:bg-white/5 p-3 rounded-lg border border-gray-200 dark:border-white/10 backdrop-blur-sm">
                        ...
                        <div className="text-[10px] text-gray-500 dark:text-gray-600 uppercase tracking-wider font-semibold">
                            Pro Score
                        </div>
                    </div>
                </div>
                    ...
                    {score.breakdown.map((item) => (
                        <div key={item.label}>
                            <div className="flex justify-between text-sm mb-1">
                                ...
                                <span className="font-bold text-gray-900 dark:text-white">{item.score}/10</span>
                            </div>
                ...
                {/* Bottom Insight */}
                <div className="mt-6 pt-4 border-t border-gray-200 dark:border-white/10 flex items-start gap-3">
