...
export default function TopPicksSidebar({ category }: { category: string }) {
    ...
    return (
        <div className="space-y-6 sticky top-24">
                    ...
                    <Card key={p.id} className="p-4 border-slate-100 hover:border-teal-200 transition-colors group">
                        <div className="flex gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <h4 className="font-bold text-sm text-slate-900 truncate group-hover:text-teal-700 transition-colors">
                                    {p.name}
                                </h4>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-50">
                            <Link href={`/go/${p.slug}`} target="_blank">
                                <Button size="sm" variant="ghost" className="w-full text-teal-700 hover:text-teal-800 hover:bg-teal-50 text-xs font-bold justify-between px-2">
            ...
            <div className="p-6 bg-gradient-to-br from-teal-600 to-teal-800 rounded-2xl text-white shadow-lg shadow-teal-500/20">
                <h4 className="font-bold mb-2">Not sure?</h4>
                <p className="text-xs text-teal-100 mb-4 leading-relaxed">Try our comparison engine to see products side-by-side.</p>
                <Link href="/products">
                    <Button size="sm" className="w-full bg-white text-teal-700 hover:bg-teal-50 font-bold border-none shadow-none">
                        Compare Now
                    </Button>
                </Link>
            </div>
