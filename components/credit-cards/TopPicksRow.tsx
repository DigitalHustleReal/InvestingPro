...
export default function TopPicksRow() {
  ...
  React.useEffect(() => {
    const fetchWinners = async () => {
      try {
        ...
        const bestTravel =
          cards.find((c: any) => c.type === "travel" && c.rating >= 4.5) ||
          cards[1];
        const bestCashback =
          cards.find((c: any) => c.type === "cashback") || cards[2];
          ...
          setWinners([
            {
              ...
              icon: Trophy,
              benefits: bestOverall.features?.slice(0, 3) || [
                "Great Rewards",
                "Low Fee",
              ],
            ...
            },
            {
              ...
              icon: Plane,
              benefits: bestTravel?.features?.slice(0, 3) || [
                "Lounge Access",
                "Air Miles",
              ],
            ...
            },
            {
              ...
              icon: ShoppingBag,
              benefits: bestCashback?.features?.slice(0, 3) || [
                "Cashback",
                "Discounts",
              ],
  ...
  if (loading)
    return (
      <div className="h-64 animate-pulse bg-gray-100 dark:bg-gray-800 rounded-2xl mb-12"></div>
    );
        ...
        {winners.map((card, idx) => (
          ...
          >
            ...
            <div className="relative h-full bg-white dark:bg-gray-900 rounded-[1.75rem] p-6 border border-gray-100 dark:border-gray-800 shadow-sm group-hover:shadow-xl transition-all duration-300 flex flex-col">
              {/* Header Badge */}
              <div
                className={`absolute top-0 right-0 ${card.badgeColor} text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-bl-xl rounded-tr-[1.75rem]`}
              >
                {card.badge}
              </div>
              ...
              <ul className="space-y-2 mb-6 flex-1">
                {card.benefits.map((benefit: string, i: number) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-xs text-gray-600 dark:text-gray-400 font-medium leading-relaxed"
                  >
              ...
              <Link href={`/credit-cards/${card.id}`} className="w-full">
                <Button
                  size="sm"
                  className="w-full rounded-xl font-bold bg-gray-900 dark:bg-white text-white dark:text-gray-900 hover:bg-primary-600 dark:hover:bg-primary-400 dark:hover:text-white transition-colors"
                >
                  Apply Now
                </Button>
              </Link>
              <p className="text-[10px] text-gray-400 dark:text-gray-500 text-center mt-1">
                No impact on CIBIL score · 3-5 min
              </p>
            </div>
          </motion.div>
        ))}
