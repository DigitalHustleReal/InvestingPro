      ...
      <div className="relative z-10 max-w-[1200px] mx-auto">
        <div className="lg:grid lg:grid-cols-2 lg:gap-10 lg:items-start">
          ...
          <div className="mb-8 lg:mb-0">
            <div className="text-xs font-semibold text-[--v2-saffron] uppercase tracking-wider mb-2">Weekly Briefing</div>
            <h2 className="text-2xl font-bold mb-2 leading-tight">The Monday briefing for smarter money decisions</h2>
            <p className="text-[13px] text-white/40 leading-relaxed mb-4">
              Rate changes, new launches, one tax tip. 60 seconds. Free forever. No spam.
            </p>
          ...
          <div className="bg-white/[.04] border border-white/[.08] rounded-xl p-4">
            <div className="text-[10px] uppercase tracking-wider text-[--v2-saffron] font-semibold mb-1.5">This Week&apos;s Edition</div>
            <h3 className="text-sm font-medium mb-1">SBI cut FD rates. RBI held steady. Here&apos;s what to do.</h3>
            <p className="text-xs text-white/35 leading-relaxed">
              Lock FD at Shriram (8.35%) · Refinance if loan &gt;9% · 42 days to ELSS deadline
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 pt-10 border-t border-white/[.06] grid gap-5 grid-cols-1 lg:grid-cols-3">
          {TRUST_ITEMS.map((item) => (
            <div key={item.title} className="pl-3.5 border-l-2 border-[--v2-saffron]">
              <h3 className="text-[13px] font-semibold mb-1">{item.title}</h3>
              <p className="text-xs text-white/35 leading-relaxed">{item.body}</p>
              <Link href={item.link} className="inline-block mt-1.5 text-xs text-[--v2-saffron] font-medium">
                {item.linkText}
              </Link>
            </div>
          ))}
