      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="text-[28px] sm:text-[36px] font-black leading-[1.0] tracking-tight text-gray-900 dark:text-white">
              What we&apos;re reading{" "}
              <span className="text-amber-600">this week.</span>
            </h2>
          </div>
          <Link
            href="/articles"
        ...
        <div className="grid gap-0 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden">
          {items.map((item, i) => (
            ...
            >
              <span className="text-[10px] uppercase tracking-wider text-green-600 mb-2 block">
                {item.tag}
              </span>
              <h3 className="text-[15px] font-bold text-gray-900 dark:text-white leading-snug mb-2 group-hover:text-green-600 transition-colors line-clamp-2">
                {item.title}
              </h3>
              ...
              {item.date && (
                <div className="mt-3 text-[10px] text-gray-300 dark:text-white/30 uppercase tracking-wider">
                  {item.date}
                </div>
              )}
            </Link>
