        ...
        <div className="flex flex-col-reverse lg:flex-row items-center gap-10 lg:gap-20">
          ...
          <div className="flex-1 text-center lg:text-left max-w-md">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-green-50 border border-green-200 rounded-full mb-5">
              ...
              <span className="text-[11px] font-semibold text-green-700 uppercase tracking-wider">Coming Soon</span>
            </div>
            ...
            <p className="text-[15px] text-gray-500 leading-relaxed mb-8">
              Monitor your net worth across mutual funds, FDs, and PPF. Get instant rate change alerts. Run any calculator on the go. Free forever.
            </p>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4 mb-8">
              {[
                ...
                <div key={f.label} className="flex items-center gap-2.5">
                  ...
                  <span className="text-[13px] text-gray-700 font-medium">{f.label}</span>
                </div>
              ))}
            </div>
            ...
            <p className="mt-3 text-[11px] text-gray-400">Works on any device. No app store needed.</p>
          </div>
                ...
                <div className="rounded-[35px] overflow-hidden bg-white">
                  ...
                  <div className="bg-[#0A1F14] px-4 pb-4 pt-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-[12px] text-white font-bold tracking-tight">InvestingP₹o</p>
                        <p className="text-[9px] text-white/35 mt-0.5">Good morning</p>
                      </div>
                  ...
                  <div className="mx-3 mt-3 p-3.5 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-xl">
                    <p className="text-[8px] text-green-600 font-semibold uppercase tracking-wider">Total Net Worth</p>
                    <p className="text-[20px] font-black text-gray-900 tracking-tight mt-0.5">₹24,85,200</p>
                    <div className="flex items-center gap-1 mt-0.5">
                      <TrendingUp size={10} className="text-green-600" />
                      <span className="text-[9px] font-semibold text-green-600">+₹1,89,400 (+8.2%)</span>
                    </div>
                    {/* Breakdown bars */}
                    <div className="mt-3 space-y-2">
                      {[
                        { label: 'Mutual Funds', value: '₹14.2L', pct: 57, color: 'bg-green-500' },
                        { label: 'Fixed Deposits', value: '₹6.8L', pct: 27, color: 'bg-emerald-400' },
                        { label: 'PPF + NPS', value: '₹3.7L', pct: 16, color: 'bg-green-300' },
                      ].map((item) => (
                        <div key={item.label}>
                          <div className="flex justify-between mb-0.5">
                            <span className="text-[8px] text-gray-600">{item.label}</span>
                            <span className="text-[8px] font-bold text-gray-900">{item.value}</span>
                          </div>
                  ...
                  <div className="mx-3 mt-2.5 p-3 bg-white border border-gray-100 rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-[9px] font-semibold text-gray-900">Rate Alerts</p>
                      <span className="text-[8px] text-green-600 font-medium">View all</span>
                    </div>
                    {[
                      { bank: 'SBI', type: 'FD 1yr', rate: '6.90%', delta: '-0.20%', down: true },
                      { bank: 'HDFC', type: 'Home Loan', rate: '8.50%', delta: '0.00%', down: false },
                      { bank: 'Axis', type: 'CC Rewards', rate: '5x pts', delta: 'New', down: false },
                    ].map((r) => (
                      <div key={r.bank + r.type} className="flex items-center justify-between py-1.5 border-t border-gray-50">
                        <div className="flex items-center gap-2">
                          <div className="w-5 h-5 rounded bg-gray-100 flex items-center justify-center">
                            <span className="text-[7px] font-bold text-gray-600">{r.bank[0]}</span>
                          </div>
                          <div>
                            <p className="text-[9px] font-semibold text-gray-900">{r.bank} {r.type}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] font-bold text-gray-900">{r.rate}</p>
                          <p className={`text-[7px] font-semibold ${r.down ? 'text-red-500' : 'text-gray-400'}`}>{r.delta}</p>
                        </div>
                      </div>
                    ))}
                  </div>
