...
export function CardComboOptimizerComponent({
  cards,
  onOptimize,
}: CardComboOptimizerProps) {
  ...
  const getCategoryIcon = (category: string) => {
    if (
      ...
      category.toLowerCase().includes("flipkart")
    ) {
              ...
              <div key={cat.key} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold">
                    ₹
                    {spending[
                      cat.key as keyof typeof spending
                    ].toLocaleString()}
                  </span>
                </div>
                <Slider
                  ...
                  onValueChange={([value]) =>
                    setSpending((prev) => ({ ...prev, [cat.key]: value }))
                  }
                  max={50000}
                  step={1000}
                  className="w-full"
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950 dark:to-emerald-950 border border-green-200 dark:border-green-800 rounded-lg text-center">
                ...
                <p className="text-2xl font-bold text-green-600">
                  ₹{result.totalAnnualValue.toLocaleString()}
                </p>
              </div>
              <div className="p-4 bg-muted rounded-lg text-center">
                ...
                <p className="text-2xl font-bold">
                  ₹{result.vsingleCardValue.toLocaleString()}
                </p>
              </div>
              <div className="p-4 bg-gradient-to-br from-blue-50 to-green-50 dark:from-blue-950 dark:to-green-950 border border-blue-200 dark:border-blue-800 rounded-lg text-center">
                <p className="text-xs text-muted-foreground uppercase tracking-wide">
                  Extra Earnings
                </p>
                <p
                  className={`text-2xl font-bold ${result.additionalValue >= 0 ? "text-blue-600" : "text-red-600"}`}
                >
              ...
              <div className="space-y-3">
                {result.usageStrategy.map((strategy, index) => (
                  ...
                  >
                    {/* Priority Badge */}
                    <div
                      className={`absolute top-0 left-0 right-0 h-1.5 ${getPriorityColor(strategy.priority)}`}
                    />
