...
export const metadata: Metadata = {
  ...
  description: 'Compare demat accounts from discount and full-service brokers. Brokerage charges, platform features, AMC fees. Independent ratings.',
  openGraph: { title: 'Best Demat Accounts in India (2026)', url: 'https://investingpro.in/demat-accounts' },
};
    ...
    <>
      ...
      <section className="bg-white border-b border-gray-200">
        <div className="max-w-[1200px] mx-auto px-4 lg:px-8 pt-6 pb-8">
          <nav aria-label="Breadcrumb" className="mb-5"><ol className="flex items-center gap-1.5 text-[13px] text-gray-400"><li><Link href="/" className="hover:text-green-600 transition-colors">Home</Link></li><li><ChevronRight size={12} /></li><li className="text-gray-700 font-medium">Demat Accounts</li></ol></nav>
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl md:text-[32px] font-bold text-[--v2-ink] tracking-tight leading-tight">Best Demat Accounts in India</h1>
              <p className="text-[15px] text-gray-500 mt-2 max-w-xl leading-relaxed">Compare brokerage charges, platform features, and account opening fees across discount and full-service brokers.</p>
            </div>
            <div className="flex items-center gap-5 text-[12px] text-gray-500 flex-shrink-0 mt-1">
              <span className="flex items-center gap-1.5"><Shield size={13} className="text-green-600" />SEBI-registered brokers</span>
              <span className="flex items-center gap-1.5"><CalendarDays size={13} className="text-green-600" />Updated monthly</span>
            </div>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {['All Brokers','Discount','Full Service','Zero Brokerage','Best for Beginners','Best for Trading'].map((p, i) => (
              <Link key={p} href={i === 0 ? '/demat-accounts' : `/demat-accounts?type=${p.toLowerCase().replace(/ /g, '-')}`} className={`inline-flex items-center px-4 py-2 rounded-full text-[13px] font-medium whitespace-nowrap transition-colors ${i === 0 ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{p}</Link>
            ))}
          </div>
        </div>
      </section>
      <section className="bg-gray-50 min-h-screen"><div className="max-w-[1200px] mx-auto px-4 lg:px-8 py-8"><DematAccountsClient /></div></section>
      <section className="bg-white border-t border-gray-200"><div className="max-w-[1200px] mx-auto px-4 lg:px-8 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[{ label: 'Brokerage Calculator', desc: 'Compare actual trading costs', href: '/calculators/brokerage' },{ label: 'Compare Brokers', desc: 'Side-by-side feature comparison', href: '/demat-accounts/compare' },{ label: 'IPO Calendar', desc: 'Upcoming IPOs and allotment status', href: '/stocks/ipo' },{ label: 'Demat Guide', desc: 'Everything about demat accounts', href: '/demat-accounts/guide' }].map((t) => (
      ...
      <section className="bg-gray-50 border-t border-gray-200"><div className="max-w-[1200px] mx-auto px-4 lg:px-8 py-10">
        <h2 className="text-lg font-bold text-[--v2-ink] mb-5">Demat Account FAQs</h2>
        <div className="space-y-2">
          {[
            { q: 'What is a demat account?', a: 'A demat (dematerialized) account holds your shares and securities in electronic form. You need one to buy stocks, mutual funds, bonds, and ETFs in India.' },
            { q: 'What is the difference between discount and full-service brokers?', a: 'Discount brokers (Zerodha, Groww) charge flat ₹20/trade or zero brokerage. Full-service brokers (ICICI Direct, HDFC Securities) charge percentage-based fees but offer research and advisory.' },
            { q: 'How much does a demat account cost?', a: 'Account opening is free at most brokers. Annual AMC (maintenance charge) ranges from ₹0 to ₹750. Trading charges vary — ₹0 to ₹20 per trade for delivery.' },
            { q: 'Can I have multiple demat accounts?', a: 'Yes. You can have demat accounts with multiple brokers. There is no legal limit. Some investors use different brokers for trading vs long-term investing.' },
            { q: 'What documents do I need to open a demat account?', a: 'PAN card, Aadhaar (for eKYC), bank account details, and a recent photograph. Most brokers offer instant digital account opening.' },
            { q: 'How does InvestingPro compare demat accounts?', a: 'We evaluate brokerage charges, AMC fees, platform features, mobile app quality, research tools, and customer support. No broker pays for higher placement.' },
          ].map((f, i) => (
