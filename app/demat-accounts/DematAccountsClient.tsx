...
import { Search, Star, ArrowRight, LayoutGrid, List, ArrowUpDown } from 'lucide-react';
...
export default function DematAccountsClient() {
    ...
    const toggleSort = (k: string) => { if (sortKey === k) setSortDir(d => d === 'desc' ? 'asc' : 'desc'); else { setSortKey(k); setSortDir(k === 'name' ? 'asc' : 'desc'); } };
    ...
    const filtered = brokers.filter((b: any) => (b.name || '').toLowerCase().includes(searchTerm.toLowerCase()));
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white">
                    <table className="w-full text-sm">
                        <thead><tr className="border-b border-gray-100 bg-gray-50/50">
                            <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider"><SH label="Broker" k="name" /></th>
                            ...
                            <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Brokerage</th>
                            <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">AMC</th>
                            <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Opening Fee</th>
                            <th className="text-right px-4 py-3" />
                        </tr></thead>
                        <tbody className="divide-y divide-gray-100">
                            {filtered.slice(0, visibleCount).map((broker: any) => (
                                <tr key={broker.id || broker.slug} className="hover:bg-green-50/30 transition-colors">
                                    <td className="px-4 py-3">
                                        <Link href={`/demat-accounts/${broker.slug || broker.id}`} className="group">
                                            <p className="text-sm font-semibold text-gray-900 group-hover:text-green-700 transition-colors">{broker.name}</p>
                                            <p className="text-[11px] text-gray-500">{broker.type || 'Discount Broker'}</p>
                                        </Link>
                                    </td>
                                    <td className="px-3 py-3 text-center">
                                        <div className="flex gap-0.5 justify-center">
                                            {[...Array(5)].map((_, i) => <Star key={i} size={11} className={i < (broker.rating || 0) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'} />)}
                                        </div>
                                    </td>
                                    <td className="px-3 py-3 text-right"><span className="text-sm font-bold text-green-600">{broker.specs?.brokerage || broker.brokerage || '₹20/trade'}</span></td>
                                    <td className="px-3 py-3 text-right"><span className="text-sm text-gray-700">{broker.specs?.amc || broker.amc || '₹0'}</span></td>
                                    <td className="px-3 py-3 text-right"><span className="text-sm text-gray-700">{broker.specs?.accountOpening || 'Free'}</span></td>
                                    <td className="px-4 py-3 text-right"><Link href={`/demat-accounts/${broker.slug || broker.id}`} className="text-xs font-medium text-green-600 hover:text-green-700">Details →</Link></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    {filtered.slice(0, visibleCount).map((broker: any) => (
                        <Link key={broker.id || broker.slug} href={`/demat-accounts/${broker.slug || broker.id}`} className="bg-white border border-gray-200 rounded-xl p-4 hover:border-green-500 hover:shadow-sm transition-all group">
                            <div className="flex items-start justify-between mb-2">
                                ...
                                <div className="flex gap-0.5">{[...Array(5)].map((_, i) => <Star key={i} size={10} className={i < (broker.rating || 0) ? 'text-amber-400 fill-amber-400' : 'text-gray-200'} />)}</div>
                            </div>
