...
const mapToRichProduct = (card: any): RichProduct & Record<string, any> => {
  ...
  return {
    ...
    features: features,
    key_features: Object.entries(features).map(([k, v]) => ({
      label: k,
      value: String(v),
    })),
...
function CreditCardCompareContent() {
    ...
    const fetchAllCards = async () => {
      try {
        const res = await fetch(
          "/api/v1/products?category=credit_card&limit=200",
        );
        if (!res.ok) return;
        const json = await res.json();
        const cards = json.products || json.data || json || [];
        if (!cancelled && Array.isArray(cards)) {
          setAllCards(cards);
        }
      } catch {
        // Silently fail — cards just won't load
      }
    };
    fetchAllCards();
    return () => {
  ...
  // Set selected cards from URL params when allCards loads
  useEffect(() => {
    ...
    if (cardIds.length > 0) {
      const matched = allCards
        .filter(
          (card: any) =>
            cardIds.includes(card.id) || cardIds.includes(card.slug),
        )
        .map(mapToRichProduct);
      setSelectedCards(matched);
    }
  }, [allCards, searchParams]);

  // Update available cards when selection changes
  useEffect(() => {
      ...
      <div className="bg-card text-card-foreground py-12 border-b border-border pt-32">
        <div className="container mx-auto px-6">
          ...
          <p className="text-muted-foreground">
            Side-by-side comparison of features, fees, and rewards
          </p>
        </div>
      </div>
        ...
        <div className="mt-12 bg-primary/5 rounded-sm p-8 border border-primary/10">
          ...
          <div className="grid md:grid-cols-3 gap-6 text-sm text-foreground/80">
            <div>
              ...
              <p>
                Travel frequently? Choose travel cards. Shop online? Go for
                cashback cards.
              </p>
            </div>
            <div>
              ...
              <p>
                High fee cards often have better rewards. Calculate if the
            ...
            <div>
              ...
              <p>
                Ensure you meet the minimum credit score and income
                requirements.
              </p>
            </div>
          </div>
