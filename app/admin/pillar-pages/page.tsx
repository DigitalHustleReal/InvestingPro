...
export default function PillarPagesPage() {
    ...
    queryFn: async () => {
      ...
      return allArticles.filter(
        (article: any) =>
          ...
          article.content_type === "category-page",
      );
    },
    initialData: [],
  });
  ...
  const categoryPages = articles.filter(
    (a: any) => a.content_type === "category-page",
  );

  const getStatusVariant = (
    status: string,
  ): "neutral" | "completed" | "processing" | "warning" | "error" => {
    switch (status) {
      case "published":
        return "completed";
      case "review":
    ...
    <AdminLayout>
      <div className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Pillar Pages"
            value={pillarPages.length}
            icon={Target}
            color="purple"
          />
          <StatCard
            label="Category Pages"
            value={categoryPages.length}
            icon={FileText}
            color="blue"
          />
          <StatCard
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {articles.map((article: any) => (
              <Link
                key={article.id}
                href={`/admin/pillar-pages/${article.id}/edit`}
              >
                <ContentSection>
                  <div className="flex flex-col md:flex-row items-start gap-4 hover:opacity-90 transition-opacity cursor-pointer -m-6 p-6">
                    <div
                      className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                        article.content_type === "pillar"
                          ...
                          : "bg-gradient-to-br from-secondary-500/20 to-emerald-500/20 border border-secondary-500/30"
                      }`}
                    >
                      {article.content_type === "pillar" ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        <StatusBadge
                          status={getStatusVariant(article.status || "draft")}
                        >
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground/70 dark:text-muted-foreground/70">
                        ...
                        {article.pillar_related_articles &&
                          article.pillar_related_articles.length > 0 && (
                            <div className="text-primary-400">
                              {article.pillar_related_articles.length} related
                              articles
                            </div>
                          )}
                      </div>
