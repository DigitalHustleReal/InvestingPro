...
export default function RevenueDashboardPage() {
    ...
    const { data: dashboardData, isLoading: dashboardLoading, refetch: refetchDashboard } = useQuery<RevenueDashboardData>({
        queryKey: ['revenue-dashboard', timeRange],
        queryFn: async () => {
            const response = await fetch(`/api/v1/admin/revenue/dashboard?startDate=${getStartDate(timeRange)}&endDate=${new Date().toISOString()}`);
    ...
    const { data: categoryData, isLoading: categoryLoading } = useQuery<CategoryData>({
        ...
        queryFn: async () => {
            ...
            const response = await fetch(`/api/v1/admin/revenue/by-category?category=${selectedCategory}&startDate=${getStartDate(timeRange)}&endDate=${new Date().toISOString()}`);
    ...
    const totalCategoryRevenue = totalByCategory.creditCards + totalByCategory.mutualFunds + totalByCategory.insurance + totalByCategory.others;
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                                    <button
                                        onClick={() => setSelectedCategory(selectedCategory === 'credit-cards' ? null : 'credit-cards')}
                                    ...
                                    <button
                                        onClick={() => setSelectedCategory(selectedCategory === 'mutual-funds' ? null : 'mutual-funds')}
                                        className={`p-4 rounded-lg border-2 transition-all ${
                                        ...
                                        {topArticlesData.articles.map((article: any, idx: number) => (
                                            ...
                                            >
                                                ...
                                                <div className="text-right ml-4">
                                                    <div className="font-bold text-lg text-sky-600 dark:text-sky-400">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    <div>
                                        <h4 className="text-sm font-medium mb-2">Daily Revenue (Last 30 Days)</h4>
                                        <div className="h-32 flex items-end gap-1">
                                            {dashboardData?.trends.daily.map((day, idx) => {
                                                const maxRevenue = Math.max(...(dashboardData?.trends.daily.map(d => d.revenue) || [1]));
