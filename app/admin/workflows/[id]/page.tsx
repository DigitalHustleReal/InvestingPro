...
export default function WorkflowDetailPage() {
    ...
    const [triggerType, setTriggerType] = useState<TriggerType>('article_created');
    ...
    useEffect(() => {
        if (workflow) {
            ...
            setConditions((workflow.conditions || []).map((c: any, i: number) => ({
                id: `cond_${i}`,
                ...c,
            })));
            setActions((workflow.actions || []).map((a: any, i: number) => ({
                id: `action_${i}`,
                ...a,
            })));
    ...
    const saveMutation = useMutation({
        mutationFn: async () => {
            ...
            if (actions.length === 0) validationErrors.push('At least one action is required');
    ...
    const getStatusIcon = (status: string) => {
        switch (status) {
            case 'completed': return <CheckCircle2 className="w-4 h-4 text-success-400" />;
            case 'running': return <RefreshCw className="w-4 h-4 text-primary-400 animate-spin" />;
            case 'failed': return <XCircle className="w-4 h-4 text-danger-400" />;
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="text-center p-3 bg-muted/10 rounded-lg">
                                        <div className="text-2xl font-bold text-foreground">{workflow.total_runs}</div>
                                        <div className="text-xs text-muted-foreground">Total Runs</div>
                                    </div>
                                    <div className="text-center p-3 bg-muted/10 rounded-lg">
                                        <div className="text-2xl font-bold text-success-400">{workflow.successful_runs}</div>
                                        <div className="text-xs text-muted-foreground">Successful</div>
                                    </div>
                                    <div className="text-center p-3 bg-muted/10 rounded-lg">
                                        <div className="text-2xl font-bold text-danger-400">{workflow.failed_runs}</div>
