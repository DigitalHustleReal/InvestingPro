...
export default function AutomationPage() {
  const { data: pipelineStatus = { completed: 0, failed: 0, lastRun: null } } =
    useQuery({
  ...
  return (
    <AdminLayout>
      <div className="p-8 space-y-8 max-w-7xl mx-auto">
        {/* Hero / Header Section */}
        <div className="relative overflow-hidden rounded-2xl p-8 bg-gray-900 border border-gray-800 shadow-xl">
          <div className="absolute top-0 right-0 -mr-16 -mt-16 h-64 w-64 rounded-full bg-emerald-500/10 blur-3xl opacity-50" />
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-gray-400 max-w-md">
                  Orchestrate and monitor your AI-driven content engine from a
          ...
          <StatCard
            ...
            value={
              pipelineStatus.lastRun
                ? new Date(pipelineStatus.lastRun).toLocaleTimeString()
                : "Idle"
            }
            icon={Clock}
            color="blue"
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    Currently operating at Level 4 Autonomy
                  </p>
                </div>
              </div>
...
function ActionCard({ title, desc, href, icon: Icon, color }: any) {
  return (
    <Link href={href}>
      <div className="group p-5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl hover:border-emerald-500/30 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 shadow-sm">
        <div
          className={cn(
  ...
  return (
    <Link href={href}>
      <div className="flex items-center justify-between p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
        <div className="flex items-center gap-4">
          <div className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800 group-hover:bg-emerald-500/10 transition-colors">
            <Icon className="w-4 h-4 text-gray-400 group-hover:text-emerald-500" />
          </div>
          <div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded-full border border-gray-200 dark:border-gray-700">
            {count}
          </span>
          <Play className="w-3 h-3 text-gray-400 group-hover:text-emerald-500 opacity-0 group-hover:opacity-100 transition-all" />
        </div>
      </div>
    </Link>
  );
