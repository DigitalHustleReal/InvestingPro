...
import { 
    Save, Loader2, ArrowLeft, Eye, Globe, 
...
export default function NewArticlePage() {
    ...
    const handlePublish = async (metadata: any = {}) => {
        const confirmed = window.confirm('Publish this article? It will be visible to all users.');
        if (!confirmed) return;
            ...
            <div className="flex flex-col h-screen bg-gray-50 dark:bg-surface-darkest transition-colors duration-300">
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                {(isAutoSaving || hasUnsavedChanges) && (
                                    <div className="flex items-center gap-2 text-xs text-muted-foreground/70" role="status">
                                        {isAutoSaving ? (
                                            <>
                ...
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40">
                    <div className="bg-gray-900 dark:bg-gray-800 text-white p-2 px-4 rounded-full shadow-2xl flex items-center gap-4 border border-gray-700">
                        ...
                        >
                            {isProofreading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                            Proofread & Polish
                        </Button>
                    </div>
                </div>
