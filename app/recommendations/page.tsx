...
} from "@/lib/ranking/recommendation-engine";
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <Card
                  key={item.id}
                  onClick={() => {
                    updatePref("productType", item.id);
                    handleNext();
                  }}
                ...
                >
                  <CardContent className="p-8 flex flex-col items-center text-center">
                    <item.icon
                      className={`w-12 h-12 mb-4 ${prefs.productType === item.id ? "text-primary-600" : "text-gray-600"}`}
                    />
      ...
      <section className="bg-gradient-to-br from-[#0B1221] via-secondary-900 to-[#0B1221] text-white pt-12 pb-32 relative overflow-hidden">
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-primary-400 font-bold uppercase text-xs tracking-wider">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Tell us what you need in 3 simple steps. We'll find the best product
            for you.
          </p>
        </div>
      </section>
        ...
        {step < 3 ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-3xl mx-auto p-8 md:p-12 min-h-[500px] flex flex-col justify-center relative">
            {step > 0 && (
              <button
                onClick={handleBack}
                className="absolute top-8 left-8 text-gray-600 hover:text-gray-600 flex items-center gap-1 text-sm font-semibold"
              >
            ...
            <div className="mb-8 flex justify-center gap-2">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className={`h-1 w-12 rounded-full transition-colors ${i <= step ? "bg-primary-600" : "bg-gray-200"}`}
                />
              ))}
            </div>
              ...
              {results.length === 0 ? (
                <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4 max-w-md mx-auto">
                    Try broadening your preferences or explore popular
                    categories below.
                  </p>
...
function OptionCard({ selected, onClick, label, icon: Icon }: any) {
  return (
    ...
    >
      <Icon
        className={`w-8 h-8 ${selected ? "text-primary-600" : "text-gray-500"}`}
      />
      <span
        className={`font-bold ${selected ? "text-primary-900" : "text-gray-700"}`}
      >
        {label}
      </span>
    </div>
