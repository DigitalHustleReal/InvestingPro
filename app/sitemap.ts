...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    // Articles/Explainer pages — Phase 3a canonical URL routing.
    // Articles with a real DB category emit /[urlCat]/learn/[slug] only;
    // articles whose category maps to "learn" (cross-cutting) keep the
    // legacy flat /articles/[slug] path. Each article appears exactly
    // once to avoid duplicate-content signals.
    try {
      const { data: articles } = await supabase
        .from("articles")
        .select("slug, category, published_date, updated_at")
        .eq("status", "published")
        .limit(5000);
