...
const AssetAllocation = dynamic(
  () => import("@/components/portfolio/AssetAllocation"),
  { ssr: false },
);
...
export default function PortfolioPage() {
  ...
  const holdings = safeHoldings.filter(
    (h: any) => h.user_email === user?.email,
  );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 bg-secondary-500/10 backdrop-blur-md rounded-full px-4 py-2 border border-secondary-500/20">
                ...
                <span className="text-secondary-400 font-semibold text- uppercase tracking-[0.2em]">
              ...
              <p className="text-xl text-gray-600 font-medium max-w-2xl leading-relaxed">
                Strategic overview of your global investment positions.
                Real-time benchmarking against India VIX and Nifty 50.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-16">
            {[
              ...
              <div
                key={i}
                className="flex items-center gap-3 bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/10 group hover:border-secondary-500/50 transition-colors"
              >
                <item.icon className="w-5 h-5 text-secondary-400 group-hover:scale-110 transition-transform" />
                <div>
                  <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-st leading-none mb-1">
                    {item.label}
                  </p>
