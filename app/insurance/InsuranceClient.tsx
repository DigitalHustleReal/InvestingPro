...
import {
  InsuranceFilterSidebar,
  InsuranceFilterState,
} from "@/components/insurance/FilterSidebar";
...
function csrBadge(csr: number | string) {
  ...
  if (!val || isNaN(val))
    return { label: "N/A", class: "bg-gray-100 text-gray-500" };
  if (val >= 98)
    return { label: `${val}% Excellent`, class: "bg-green-50 text-green-700" };
  if (val >= 95)
    return { label: `${val}% Good`, class: "bg-green-50 text-green-600" };
  if (val >= 90)
    return { label: `${val}% Fair`, class: "bg-amber-50 text-amber-700" };
...
export default function InsuranceClient({
  ...
  const filtered = assets.filter((a) => {
    ...
    const typeMatch =
      policyType === "all" ||
      (a.category || a.type || "").toLowerCase().includes(policyType);
    const csrVal = parseFloat(
      a.specs?.claimRatio || a.claimSettlementRatio || "0",
    );
    const csrMatch = csrVal >= minCSR || minCSR === 0;
    const insurerMatch =
      filters.insurers.length === 0 ||
      filters.insurers.some((i: string) => provider.includes(i.toLowerCase()));
    const sidebarTypeMatch =
      filters.policyTypes.length === 0 ||
      filters.policyTypes.some((t: string) =>
        (a.category || a.type || "").toLowerCase().includes(t.toLowerCase()),
      );
    return (
      searchMatch && typeMatch && csrMatch && insurerMatch && sidebarTypeMatch
    );
  });
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.slice(0, visibleCount).map((plan: any) => {
                  const csr = csrBadge(
                    plan.specs?.claimRatio || plan.claimSettlementRatio || 0,
                  );
                  return (
                    <tr
                    ...
                    >
                      <td className="px-4 py-3">
                        <Link
                          href={`/insurance/${plan.slug || plan.id}`}
                          className="group"
                        >
                          ...
                          <p className="text-[11px] text-gray-500">
                            {plan.provider_name || plan.providerName} ·{" "}
                            {plan.category || plan.type || "Health"}
                          </p>
                        </Link>
                      </td>
                      <td className="px-3 py-3 text-right">
                        <span className="text-sm font-bold text-gray-900 tabular-nums">
                          {plan.specs?.premium || plan.premium || "Quote"}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-right">
                        <span className="text-sm font-semibold text-gray-900">
                          {plan.specs?.coverAmount || plan.coverAmount || "N/A"}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-center">
                        <Badge className={`text-[9px] font-bold ${csr.class}`}>
                          {csr.label}
                        </Badge>
                      </td>
                      <td className="px-3 py-3 text-right">
                        <span className="text-sm text-gray-700">
                          {plan.specs?.networkHospitals || "N/A"}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-right">
                        <span className="text-sm text-gray-700">
                          {plan.specs?.claimSpeed || "5-7 days"}
                        </span>
                      </td>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3">
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900">
                        {plan.specs?.premium || plan.premium || "Get Quote"}
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-gray-900">
                        {plan.specs?.roomRentLimit || "No limit"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 mb-3 p-2.5 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      ...
                      <Badge
                        className={`text-[10px] font-bold mt-0.5 ${csr.class}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    {plan.specs?.waitingPeriod && (
                      <span className="text-[10px] text-gray-600 bg-gray-100 px-2 py-0.5 rounded">
                        Waiting: {plan.specs.waitingPeriod}
                      </span>
                    )}
                    {plan.specs?.coPayment === "0%" ||
                    plan.specs?.coPayment === 0 ? (
                      <span className="text-[10px] text-green-700 bg-green-50 px-2 py-0.5 rounded font-medium">
                        0% Co-pay
                      </span>
                    ) : plan.specs?.coPayment ? (
                      <span className="text-[10px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded">
                        {plan.specs.coPayment} Co-pay
                      </span>
                    ) : null}
                    {plan.specs?.noClaimBonus && (
                      <span className="text-[10px] text-green-700 bg-green-50 px-2 py-0.5 rounded font-medium">
                        <Heart size={9} className="inline mr-0.5" />
                        NCB: {plan.specs.noClaimBonus}
                      </span>
                    )}
                  </div>
