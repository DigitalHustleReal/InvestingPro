...
export default function ProfilePage() {
  ...
  const totalHelpful = userReviews.reduce(
    (sum: number, r: any) => sum + (r.helpful_count || 0),
    0,
  );
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      styles[status] ||
      "bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-600"
    );
  };
            ...
            <div className="flex-1 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge className="bg-primary-500 text-gray-900 font-semibold rounded-lg uppercase tracking-st text-[9px] px-2.5">
                ...
                <Button
                  ...
                  variant="outline"
                  className="rounded-2xl border-white/10 text-white hover:bg-white/5 font-semibold h-12 px-8 uppercase tracking-widest text-"
                >
              ...
              <div className="max-w-3xl">
                <p className="text-lg text-gray-300 font-medium leading-relaxed italic opacity-80 backdrop-blur-sm bg-black/10 p-4 rounded-2xl border border-white/5">
                  "
                  {user.bio ||
                    "Market enthusiast and verified InvestingPro contributor analyzing trends in the Indian financial landscape."}
                  "
                </p>
                ...
                {user.expertise?.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-6">
                    {user.expertise.map((exp: string, idx: number) => (
                      <Badge
                        key={idx}
                        className="bg-white/5 text-gray-300 border-white/10 rounded-xl px-4 py-1.5 font-semibold uppercase text-[9px] tracking-st"
                      >
                        {exp}
                      </Badge>
                    ))}
        ...
        <div className="space-y-8">
          ...
          <div className="grid lg:grid-cols-4 gap-6">
            ...
            <Card className="rounded-[2.5rem] border-0 shadow-2xl bg-white dark:bg-gray-900 p-8 overflow-hidden relative group">
              ...
              <h4 className="text-[10px] font-semibold text-gray-600 uppercase tracking-st mb-6 relative z-10">
                Achievements
              </h4>
              <div className="relative z-10">
                ...
                {(!user.badges || user.badges.length === 0) && (
                  <p className="text-xs font-bold text-gray-600 italic">
                    No badges unlocked yet. Start submitting analysis!
                  </p>
                )}
              </div>
            </Card>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, index) => (
              <Card
                key={index}
                className="rounded-[2.5rem] border-0 shadow-xl bg-white dark:bg-gray-900 p-8 group hover:-translate-y-1 transition-all"
              >
                <div
                  className={`w-12 h-12 rounded-2xl ${stat.bg} dark:bg-white/5 ${stat.color} flex items-center justify-center mb-4 shadow-sm group-hover:rotate-6 transition-transform`}
                >
                  <stat.icon className="w-5 h-5" />
                </div>
                <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-st mb-1">
                  {stat.label}
                </p>
          ...
          <Tabs defaultValue="articles" className="space-y-8">
            <TabsList className="bg-white dark:bg-gray-900 p-2 rounded-[2rem] shadow-xl border border-gray-100 dark:border-gray-800 flex h-auto max-w-fit mx-auto sm:mx-0">
              <TabsTrigger
                value="articles"
                className="rounded-3xl px-8 py-4 font-semibold uppercase tracking-widest text- data-[state=active]:bg-gray-900 dark:data-[state=active]:bg-primary-600 data-[state=active]:text-white transition-all text-gray-500 dark:text-gray-600"
              >
                Analysis Hub{" "}
            ...
            >
              {userArticles.length === 0 ? (
                <Card className="rounded-[3rem] border-0 shadow-2xl bg-white dark:bg-gray-900 p-20 text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-600 font-medium mb-10">
                    You haven't published any analysis articles yet. Share your
                    market wisdom with the community.
                  </p>
                  <Button className="rounded-2xl bg-primary-600 hover:bg-secondary-600 dark:bg-primary-500 dark:hover:bg-secondary-500 text-white font-bold h-14 px-8 shadow-xl transition-all">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {userArticles.map((article: any) => (
                    <Card
                      key={article.id}
                      className="rounded-[3rem] border-0 shadow-lg bg-white dark:bg-gray-900 overflow-hidden group hover:shadow-2xl transition-all"
                    >
                      <div className="p-8">
                        <div className="flex justify-between items-start mb-6">
                          <Badge
                            className={`${getStatusBadge(article.submission_status || article.status)} border-0 rounded-xl px-4 py-1.5 font-bold uppercase tracking-widest text-[9px]`}
                          >
                            {article.submission_status || article.status}
                          </Badge>
                          <span className="text-[10px] font-semibold text-gray-600 uppercase tracking-st">
                            {new Date(
                              article.created_at || article.created_date,
                        ...
                        <h4 className="text-xl font-bold text-gray-900 dark:text-white mb-4 tracking-tight leading-tight group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                          {article.title}
                        </h4>
                        <div className="flex items-center gap-6 pt-4 border-t border-gray-50 dark:border-gray-800">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-gray-500 dark:text-gray-600">
                              {article.views || 0}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Badge
                              variant="outline"
                              className="border-gray-100 dark:border-gray-800 font-bold text-[9px] uppercase dark:text-gray-300"
                            >
                        ...
                        {article.rejection_reason && (
                          <div className="mt-6 p-4 bg-danger-50 dark:bg-danger-900/20 rounded-2xl border border-danger-100 dark:border-danger-900/50 flex gap-3">
                            <div className="w-6 h-6 rounded-full bg-danger-500 text-white flex items-center justify-center shrink-0 text-[10px] font-bold pt-0.5">
                              !
                            </div>
              ...
              <Card className="rounded-[3rem] border-0 shadow-2xl bg-white dark:bg-gray-900 p-6 md:p-8">
                {userReviews.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 font-semibold uppercase tracking-widest text-">
                      Conflict evidence not found
                    </p>
                  </div>
                ) : (
                  <div className="grid gap-6">
                    {userReviews.map((review: any) => (
                      <div
                        key={review.id}
                        className="p-8 bg-gray-50/50 dark:bg-gray-800/50 rounded-3xl border border-gray-100 dark:border-gray-800 group hover:bg-white dark:hover:bg-gray-800 hover:shadow-xl transition-all"
                      >
                        <div className="flex items-start justify-between mb-6">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <span className="text-[10px] font-semibold text-gray-600 uppercase tracking-st">
                                {new Date(
                                  review.created_at || review.created_date,
                                ).toLocaleDateString()}
                              </span>
                            </div>
                            {review.title && (
                              <h4 className="text-lg font-bold text-gray-900 dark:text-white mb-4 tracking-tight leading-tight">
                                {review.title}
                              </h4>
                            )}
                          ...
                          <Badge
                            className={`${getStatusBadge(review.status)} border-0 rounded-xl px-4 py-1.5 font-bold uppercase tracking-widest text-[9px]`}
                          >
                            {review.status}
                          </Badge>
                        </div>
