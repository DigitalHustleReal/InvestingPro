...
interface FixedDepositsClientProps {
  initialFDs?: any[];
}

export default function FixedDepositsClient({
  initialFDs = [],
}: FixedDepositsClientProps) {
  ...
  const { data: fdRates = [], isLoading } = useQuery({
    ...
    initialData: initialFDs.length > 0 ? initialFDs : undefined,
    staleTime: initialFDs.length > 0 ? 60 * 1000 : 0,
  });
  ...
  const sorted = [...filtered].sort((a: any, b: any) =>
    sortBy === "rate"
      ? (b.interestRate || b.rate || 0) - (a.interestRate || a.rate || 0)
      : 0,
  );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input
            ...
            aria-label="Search fixed deposits"
          />
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
            No fixed deposits match your filters
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Try clearing your filters or adjusting tenure and amount
            preferences.
          </p>
          <a
            href="/fixed-deposits"
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    <p className="text-[11px] text-gray-500">
                      {fd.type || "Fixed Deposit"}
                    </p>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <span className="text-sm font-bold text-green-600 tabular-nums">
                      {fd.interestRate || fd.rate || "N/A"}%
                    </span>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <span className="text-sm font-semibold text-gray-700 tabular-nums">
                      {fd.seniorRate || fd.seniorCitizenRate || "—"}%
                    </span>
                  </td>
                  <td className="px-3 py-3 text-right">
                    <span className="text-sm text-gray-600">
                      {fd.tenure || fd.minTenure || "N/A"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <Link
                      href={`/fixed-deposits/${fd.slug || fd.id}`}
          ...
          {sorted.slice(0, visibleCount).map((fd: any) => (
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-2 pt-3 mt-3 border-t border-gray-100 text-center">
                <div>
                  <p className="text-[9px] text-gray-400 uppercase">Regular</p>
                  <p className="text-lg font-bold text-green-600 tabular-nums">
                    {fd.interestRate || fd.rate || "N/A"}%
                  </p>
                </div>
                <div>
                  <p className="text-[9px] text-gray-400 uppercase">Senior</p>
                  <p className="text-lg font-bold text-gray-700 tabular-nums">
                    {fd.seniorRate || fd.seniorCitizenRate || "—"}%
                  </p>
                </div>
                <div>
                  <p className="text-[9px] text-gray-400 uppercase">Tenure</p>
              ...
              <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-50">
                <span className="text-[11px] text-gray-500">
                  {fd.taxSaving ? "Tax saving u/s 80C" : "Standard FD"}
                </span>
