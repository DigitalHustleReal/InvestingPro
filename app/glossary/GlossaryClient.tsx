...
interface GlossaryClientProps {
  /**
   * Server-fetched, locale-aware initial term set. When supplied,
   * the client skips the legacy `api.entities.Glossary.list()` call
   * — that path returned only English. Phase 3a wires the page to
   * server-side `getGlossaryIndex(locale)`; this prop carries the
   * result down.
   */
  initialTerms?: GlossaryTerm[];
  /** Resolved request locale, used to localise the term link hrefs. */
  locale?: Locale;
}
...
export default function GlossaryPage({
  ...
  const hasInitial = !!initialTerms && initialTerms.length > 0;
  const [terms, setTerms] = useState<GlossaryTerm[]>(initialTerms ?? []);
  const [loading, setLoading] = useState(!hasInitial);

  useEffect(() => {
    // When the page passed locale-aware initial terms, skip the
    // legacy English-only fetch. Only fall back to the API when
    // SSR didn't supply data (defensive — should never happen
    // post-Phase-3a but keeps the component standalone).
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400 mb-6">
              {filteredTerms.length} terms to help you understand the financial
              world
            </p>
                              ...
                              <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <Badge
                                      className={`text-xs px-2 py-0.5 ${CATEGORY_CONFIG[term.category]?.color || "bg-gray-100 text-gray-600"}`}
                                    >
                                      {term.category}
                                    </Badge>
                                  </div>
