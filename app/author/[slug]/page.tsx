...
export default async function AuthorProfilePage({ params }: PageProps) {
  ...
  const { data: articles } = await supabase
    .from("articles")
    .select(
      "id, title, slug, excerpt, featured_image, published_date, category",
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <SEOHead
        ...
        description={
          author.bio ||
          `Read articles and insights from ${author.name}, ${author.role || "financial expert"} at InvestingPro.`
        }
      />

      {/* Header / Cover */}
      <div className="relative h-64 md:h-80 bg-gray-900 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-primary-900 via-gray-900 to-gray-950"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 relative z-10 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden sticky top-24 border border-gray-100 dark:border-gray-800">
              <div className="p-8 flex flex-col items-center text-center">
                <div className="w-32 h-32 rounded-full border-4 border-white dark:border-gray-800 shadow-lg overflow-hidden mb-6 bg-gray-100 dark:bg-gray-800">
                  {author.avatar_url ? (
                    <Image
                ...
                <div className="w-full grid grid-cols-2 gap-2 border-t border-gray-100 dark:border-gray-800 pt-6">
          ...
          <div className="lg:col-span-2 space-y-8 mt-4 lg:mt-32">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-xl p-8 shadow-sm border border-gray-200 dark:border-gray-800">
              ...
              {author.specialization && author.specialization.length > 0 && (
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
                  <p className="text-sm font-semibold text-gray-900 dark:text-white mb-3 uppercase tracking-wide">
                    Areas of Expertise
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {author.specialization.map((skill: string, i: number) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 rounded-full text-sm font-medium border border-primary-100 dark:border-primary-800"
                      >
                        {skill}
                      </span>
                    ))}
            ...
            <div className="bg-primary-50 dark:bg-primary-900/20 rounded-xl p-6 border border-primary-100 dark:border-primary-800 flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-primary-900 dark:text-primary-100 mb-1">
                  Editorial Integrity Verified
                </h3>
                <p className="text-sm text-primary-800 dark:text-primary-200 leading-relaxed">
                  All content written by {author.name} adheres to InvestingPro's
                  strict editorial guidelines. Financial advice is backed by
                  data-driven research and peer-reviewed by our compliance team.
                </p>
              </div>
            </div>
                ...
                <div className="grid gap-6">
                  {articles.map((article) => (
                    <Link
                      ...
                      className="group bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden hover:shadow-md transition-all flex flex-col md:flex-row h-full md:h-48"
                    >
                      {article.featured_image && (
                        <div className="w-full md:w-64 relative h-48 md:h-full bg-gray-200 dark:bg-gray-800 shrink-0">
                          <Image
                          ...
                          />
                          {article.category && (
                            <div className="absolute top-3 left-3 bg-white/90 dark:bg-gray-900/90 backdrop-blur px-2 py-1 rounded text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wide">
                              {article.category}
                            </div>
                          )}
                        </div>
