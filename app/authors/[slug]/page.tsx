...
export default async function AuthorPage({ params }: PageProps) {
  ...
  // JSON-LD structured data — Organization for team bylines, Person for individuals
  const isTeam =
    author.name?.includes("Team") ||
    author.name?.includes("Desk") ||
    author.name?.includes("InvestingPro");
  const jsonLd = isTeam
    ? {
        ...
        knowsAbout: author.credentials || ["Personal Finance", "Investing"],
      }
    : {
        "@context": "https://schema.org",
        "@type": "Person",
        name: author.name,
        jobTitle: author.role || "Financial Analyst",
        description: author.bio,
        url: `https://investingpro.in/authors/${author.slug}`,
        worksFor: {
          "@type": "Organization",
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                ...
                <div className="text-center md:text-left flex-1">
                  ...
                  {/* Bio */}
                  {author.bio && (
                    <p className="text-primary-100 max-w-2xl">{author.bio}</p>
                  )}
