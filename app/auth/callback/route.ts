...
export async function GET(request: Request) {
  ...
  // Handle auth errors
  if (error) {
    logger.error("[AUTH CALLBACK] Auth error:", error, {
  ...
  if (!session && token_hash && type) {
    ...
    if (verifyError) {
      logger.error(
        "[AUTH CALLBACK] Magic link verification error:",
        verifyError,
      );
      return NextResponse.redirect(
  ...
  if (session?.user) {
    ...
    if (!profile) {
      ...
      if (session.user.email) {
        sendWelcomeEmail(session.user.email, userName || "there").catch(
          (err) => {
            logger.error("[AUTH CALLBACK] Welcome email failed", err);
          },
        );
      }
    }
    ...
    if (source === "admin") {
      ...
      if (!isSystemAdmin) {
        logger.warn(
          `[AUTH CALLBACK] Non-admin (${session.user.email}) tried admin login`,
        );
    ...
    logger.info(
      `[AUTH CALLBACK] ✅ ${session.user.email} (${userRole}) → ${finalRedirect}`,
    );
