...
function generateTimeSeries(range: string) {
  // Returns a zero-valued time series — no fake Math.random() data.
  // Real values will surface once the analytics_events writer is
  // wired (currently orphan; deferred per 2026-04-26 audit). The
  // dashboard chart should render an empty-state when sums are 0.
  const days =
    range === "today"
      ? 1
      : range === "7d"
