...
import {
  ...
  getAllTrackedKeywords,
} from "@/lib/seo/serp-tracker";
import { logger } from "@/lib/logger";
import { createClient, type SupabaseClient } from "@supabase/supabase-js";

// Lazy Supabase client — module-level construction with strict-env access
// would throw at import time if any required env var is missing on Vercel,
// producing an opaque 500 from the cron. We construct lazily inside the
// handler so missing env returns a clean 200 with "skipped" instead.
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase) {
      logger.warn(
        "[cron seo-rankings-update] skipped — Supabase env vars missing",
      );
    ...
    const targetKeywords = process.env.SEO_TARGET_KEYWORDS
      ? process.env.SEO_TARGET_KEYWORDS.split(",")
          .map((k) => k.trim())
          .filter(Boolean)
      : [];
