...
function mapConduitToArticle(content: Record<string, unknown>) {
  ...
  return {
    ...
    read_time: readTime,
    published_at:
      content.publish_date || content.created_at || new Date().toISOString(),
  ...
  try {
    ...
    switch (event) {
      ...
      case "content.updated": {
        ...
        if (error) {
          logger.error("Conduit sync error", undefined, {
      ...
      case "content.unpublished": {
        ...
        if (error) {
          logger.error("Conduit delete error", undefined, {
            message: error.message,
          });
          syncResult = { error: error.message };
        } else {
...
export async function GET() {
  return NextResponse.json({
    ...
    webhook: "conduit-cms",
    description:
      "Syncs Conduit CMS content to investingpro articles table + ISR revalidation",
    timestamp: new Date().toISOString(),
  });
}
