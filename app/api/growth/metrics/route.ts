...
export async function GET(request: NextRequest) {
  try {
    ...
    const [segments, engagement, trafficSources, revenueMetrics, funnel] =
      await Promise.all([
    ...
    const returningUsers =
      segments.find((s) => s.segment === "returning")?.count || 0;
    const totalVisitors = newUsers + returningUsers;

    // Calculate retention metrics
    const retentionRate =
      totalVisitors > 0 ? (returningUsers / totalVisitors) * 100 : 0;
    ...
    const averageOrderValue =
      revenueMetrics.totalConversions > 0
        ? totalRevenue / revenueMetrics.totalConversions
        : 0;

    // Trends: zeros until real analytics_events writer is wired
    // (currently orphan per 2026-04-26 audit). Admin dashboard
    // renders an empty state when these series sum to zero — this
    // is honest about the absence of data rather than synthesising
    // fake "10,000 visitors" numbers via Math.random().
    const dailyTrends = [];
    for (let i = 29; i >= 0; i--) {
      const date = new Date();
