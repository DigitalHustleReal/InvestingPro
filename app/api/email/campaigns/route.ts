/**
 * Email Campaigns API
 * Returns email campaign performance
 */
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Get email campaigns
        const campaigns = await getEmailCampaigns(startDateStr, endDateStr);
        ...
        return NextResponse.json(
            {
                ...
                code: 'INTERNAL_ERROR',
                message: 'Failed to fetch email campaigns. Please try again later.'
            },
            { status: 500 }
        );
    }
