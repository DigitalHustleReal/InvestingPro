...
import { findRelatedArticles, findLinkOpportunities } from '@/lib/automation/auto-interlinking';
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (relatedArticles.length === 0) {
            return NextResponse.json({ suggestions: [], message: 'No related articles found' });
        }
