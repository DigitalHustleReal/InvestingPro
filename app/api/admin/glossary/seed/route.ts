...
// Import the glossary terms from the script
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Map category names to schema-compliant values
      ...
      try {
        ...
        const { error } = await supabase
          ...
          .insert({
            ...
            category: categoryMap[term.category] || 'general',
            why_it_matters: `Understanding ${term.term} is crucial for making informed financial decisions in India.`,
            example_numeric: term.example || `Example: ${term.term} in practice`,
            example_text: term.example,
