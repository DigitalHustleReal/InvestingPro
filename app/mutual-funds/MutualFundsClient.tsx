...
interface MutualFundsClientProps {
  initialFunds?: any[];
}

export default function MutualFundsClient({
  initialFunds = [],
}: MutualFundsClientProps) {
  const [viewMode, setViewMode] = useState<"grid" | "table">("table");
  const [funds, setFunds] = useState<any[]>(() => {
    // Use server-provided data if available (avoids client-side fetch waterfall)
    if (initialFunds.length > 0) {
      return initialFunds.map((p: any) => ({
        id: p.id || p.slug,
        name: p.name || "Unknown Fund",
        category: p.category || "Equity",
        aum: p.aum || "N/A",
        returns_1y: p.returns_1y ?? p.returns1Y ?? 0,
        returns_3y: p.returns_3y ?? p.returns3Y ?? 0,
        returns_5y: p.returns_5y ?? p.returns5Y ?? 0,
        rating: p.rating ?? 0,
        risk: p.risk || p.riskLevel || "Moderate",
        expense_ratio: p.expense_ratio ?? p.expenseRatio ?? 0,
        min_investment: p.min_investment || p.minInvestment || "₹500",
        fund_house: p.fund_house || p.providerName || "Unknown",
      }));
    }
    return [];
  });
  const [loading, setLoading] = useState(initialFunds.length === 0);
  ...
  useEffect(() => {
    ...
    if (initialFunds.length > 0) return;
    ...
    (async () => {
      ...
      try {
        ...
        logger.error("[MutualFunds] Error", error as Error);
        setFunds([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    setVisibleCount(6);
  }, [searchTerm, filters, sortKey, sortDir]);
  ...
  const filtered = (Array.isArray(funds) ? funds : [])
    .filter((f) => {
      const searchMatch =
        ...
        (f.fund_house || "").toLowerCase().includes(searchTerm.toLowerCase());
      const matchesRisk =
        filters.riskLevels.length === 0 ||
        filters.riskLevels.includes(f.risk || "");
        ...
        {loading ? (
          ...
          <div className="py-16 text-center bg-white rounded-xl border border-gray-200">
            ...
            <p className="text-sm text-gray-500 mb-5 max-w-md mx-auto">
              Try adjusting your filters or browse popular fund categories
              instead.
            </p>
              ...
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/50">
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                    <SortHeader label="Fund Name" sortKeyName="name" />
                  </th>
                  ...
                  <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                    <SortHeader label="1Y" sortKeyName="returns_1y" />
                  </th>
                  <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                    <SortHeader label="3Y" sortKeyName="returns_3y" />
                  </th>
                  <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                    <SortHeader label="5Y" sortKeyName="returns_5y" />
                  </th>
                  <th className="text-right px-3 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-wider">
                    <SortHeader label="Exp." sortKeyName="expense_ratio" />
                  </th>
              ...
              <tbody className="divide-y divide-gray-100">
                {visible.map((fund) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-center">
                      <span
                        className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded ${fund.risk === "Low" || fund.risk === "Low to Moderate" ? "bg-green-50 text-green-700" : fund.risk === "High" || fund.risk === "Very High" ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-700"}`}
                      >
                        {fund.risk}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-right">
                      <span
                        className={`text-sm font-semibold tabular-nums ${fund.returns_1y >= 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        {fund.returns_1y}%
                      </span>
                    </td>
                    <td className="px-3 py-3 text-right">
                      <span
                        className={`text-sm font-bold tabular-nums ${fund.returns_3y >= 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        {fund.returns_3y}%
                      </span>
                    </td>
                    <td className="px-3 py-3 text-right">
                      <span
                        className={`text-sm font-semibold tabular-nums ${fund.returns_5y >= 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        {fund.returns_5y}%
                      </span>
                    </td>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-gray-900 group-hover:text-green-700 transition-colors leading-tight">
                      {fund.name}
                    </h3>
                  ...
                  <Badge
                    className={`text-[9px] font-bold px-2 py-0.5 flex-shrink-0 ${fund.risk === "Low" ? "bg-green-50 text-green-700" : fund.risk === "High" || fund.risk === "Very High" ? "bg-red-50 text-red-700" : "bg-amber-50 text-amber-700"}`}
                  >
                    {fund.risk}
                  </Badge>
                </div>
                <div className="grid grid-cols-4 gap-2 pt-3 border-t border-gray-100">
                  {[
                    { l: "1Y", v: fund.returns_1y },
                    { l: "3Y", v: fund.returns_3y },
                    { l: "5Y", v: fund.returns_5y },
                    { l: "Exp", v: fund.expense_ratio, isExp: true },
                  ].map((c) => (
                    <div key={c.l} className="text-center">
                      ...
                      <p
                        className={`text-sm font-bold tabular-nums ${c.isExp ? "text-gray-700" : c.v >= 0 ? "text-green-600" : "text-red-500"}`}
                      >
                        {c.v}%
                      </p>
                    </div>
