    ...
    if (funds.length === 0) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
                <h1 className="text-2xl font-bold text-slate-900 mb-4">No Funds Selected</h1>
                <p className="text-slate-500 mb-8">Please select mutual funds to compare.</p>
                <Link href="/">
        ...
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                <div className="container mx-auto px-6 py-4">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <h1 className="text-xl font-bold text-slate-900">Compare Funds</h1>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        {funds.map(fund => (
                            <div key={fund!.id} className="bg-slate-50 p-3 rounded-lg border border-slate-100 relative">
                                <Link href={`/mutual-funds/${fund!.id}`} className="hover:underline">
                                    <h3 className="font-bold text-slate-900 text-sm leading-tight mb-1">{fund!.name}</h3>
                                </Link>
            ...
            <div className="container mx-auto px-6 py-8">
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    ...
                    {/* Returns Section */}
                    <div className="bg-slate-50 px-6 py-3 border-y border-slate-200 font-bold text-slate-900 text-sm uppercase tracking-wide">
                        Returns Performance
                    </div>
                    <div className="divide-y divide-slate-100">
...
function ComparisonRow({ label, funds, render }: { label: string, funds: any[], render: (f: any) => React.ReactNode }) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 px-6 py-4 hover:bg-slate-50 transition-colors">
            <div className="font-medium text-slate-500 text-sm flex items-center">{label}</div>
            {funds.map(fund => (
                <div key={fund.id} className="text-slate-900 text-sm flex items-center">
                    <span className="md:hidden font-medium text-slate-500 mr-2">{label}:</span>
                    {render(fund)}
                </div>
            ))}
            {[...Array(3 - funds.length)].map((_, i) => <div key={i} className="hidden md:block"></div>)}
