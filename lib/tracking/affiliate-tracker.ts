...
export async function trackAffiliateClick(
  ...
  try {
    ...
    // Track in PostHog for analytics dashboards
    ...
    logger.error(
      "Error tracking affiliate click",
...
export async function getProductClickStats(productId: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      conversionRate:
        totalClicks > 0
          ? Math.round((conversions / totalClicks) * 100 * 10) / 10
          : 0,
    };
  } catch (error) {
    logger.error(
      "Error getting product click stats",
      error instanceof Error ? error : new Error(String(error)),
    );
    return null;
  }
...
export async function getClicksOverview(days: number = 7): Promise<{
  ...
  try {
    ...
    // Calculate stats
    const uniqueSessions = new Set((data as any[]).map((d) => d.session_id))
      .size;
