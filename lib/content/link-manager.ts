...
export function enrichContent(html: string): string {
  ...
  // Supported: [sip-calculator], [emi-calculator], [fd-calculator], [ppf-calculator],
  //            [nps-calculator], [tax-calculator], [ssy-calculator], [rd-calculator],
  //            [swp-calculator], [retirement-calculator], [compound-interest-calculator],
  //            [auto-calculator], [calculator-cta type="sip"]
  const calcWidgetClass =
    "my-8 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 text-center text-gray-500";
  const supportedCalcs = [
    "sip",
    "emi",
    "fd",
    "ppf",
  ...
  for (const calc of supportedCalcs) {
    enriched = enriched.replace(
      new RegExp(`\\[${calc}-calculator\\]`, "gi"),
      `<div data-widget="${calc}-calculator" class="${calcWidgetClass}">Loading ${calc.toUpperCase()} Calculator...</div>`,
    );
  }
  // Handle [calculator-cta type="X"] shortcode format (generated by SERP pipeline)
  enriched = enriched.replace(
    /\[calculator-cta\s+type="([^"]+)"\]/gi,
    (_, type) => {
      const widgetName = `${type.toLowerCase()}-calculator`;
      return `<div data-widget="${widgetName}" class="${calcWidgetClass}">Loading ${type.toUpperCase()} Calculator...</div>`;
    },
  );
  ...
  enriched = enriched.replace(
    /<table/gi,
    '<div class="overflow-x-auto w-full my-8 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm"><table',
  );
