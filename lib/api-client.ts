    ...
    Article: {
      list: async (
        ...
        if (includeAllStatuses) {
          const response = await fetch(
            `/api/admin/articles?limit=${limit || 500}${order ? `&order=${order}` : ""}&includeDeleted=true`,
          );
          if (!response.ok) {
      ...
      delete: async (id: string, permanent: boolean = false) => {
        ...
        if (!response.ok) {
          const error = await response
            .json()
            .catch(() => ({ message: "Failed to delete article" }));
          throw new Error(error.message || "Failed to delete article");
        }
        return await response.json();
      },
      bulkAction: async (
        action: "publish" | "archive" | "delete" | "unpublish" | "restore",
        ids: string[],
      ) => {
        ...
        if (!response.ok) {
          const error = await response
            .json()
            .catch(() => ({ message: `Failed to ${action} articles` }));
