  ...
  try {
    ...
    try {
      ...
      const telegramChatIds = process.env.TELEGRAM_CONTENT_CHANNELS
        ? process.env.TELEGRAM_CONTENT_CHANNELS.split(",")
            .map((id) => id.trim())
            .filter(Boolean)
        : [];

      const whatsappNumbers = process.env.WHATSAPP_CONTENT_CHANNELS
        ? process.env.WHATSAPP_CONTENT_CHANNELS.split(",")
            .map((num) => num.trim())
            .filter(Boolean)
        : [];
      ...
      if (telegramChatIds.length > 0 || whatsappNumbers.length > 0) {
        ...
        messagingResult = {
          telegram: {
            ...messagingResult.telegram,
            ...messagingSendResult.telegram,
          },
          whatsapp: {
            ...messagingResult.whatsapp,
            ...messagingSendResult.whatsapp,
          },
        };
      }
    } catch (error: any) {
      logger.error("Error sending messaging notification", error, {
        articleId,
      });
    ...
    // 4. Mark article as distributed (store timestamp in editorial_notes JSONB)
    try {
      // Read current editorial_notes, then merge distributed_at
      ...
      const existingNotes =
        (current?.editorial_notes as Record<string, unknown>) || {};
      await getSupabaseClient()
        ...
        .update({
          editorial_notes: {
            ...
            distribution_result: {
              twitter: socialMediaResult.twitter,
              linkedin: socialMediaResult.linkedin,
...
export async function distributeNewArticles(): Promise<{
  ...
  try {
    ...
    const { data: newArticles, error } = await getSupabaseClient()
      .from("articles")
      .select("id, editorial_notes")
      .eq("status", "published")
      .gte("published_date", yesterday.toISOString())
      .order("published_date", { ascending: false })
      .limit(20); // Fetch more, then filter already-distributed
    ...
    // Filter out articles already distributed (editorial_notes.distributed_at is set)
    const undistributed = newArticles
      .filter((a) => {
        const notes = a.editorial_notes as Record<string, unknown> | null;
        return !notes?.distributed_at;
      })
      .slice(0, 10); // Cap at 10 per run
    ...
    for (const article of undistributed) {
      try {
        ...
        logger.error("Failed to distribute article", error, {
