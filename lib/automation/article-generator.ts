...
} from "@/lib/quality/content-quality-scorer";
...
function getSupabaseClient() {
  if (!supabaseClient) {
    ...
    if (!url || !key) {
      throw new Error(
        "Supabase credentials not found. Make sure NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are in .env.local",
      );
    }
...
import {
  buildDynamicPrompt,
  type ContentType,
} from "@/lib/ai/dynamic-prompt-builder";
import {
  isValidCategory,
...
async function getArticlePrompt(
  ...
  const financeCategory =
    category &&
    isValidCategory(categoryMap[category] || (category as FinanceCategory))
  ...
  try {
    ...
    logger.warn(
      "Dynamic prompt builder failed, falling back to legacy system:",
      error,
    );
  }
  ...
  if (dbPrompt) {
    ...
    let promptText = promptService.populateTemplate(
      dbPrompt.user_prompt_template,
      {
        topic: topic,
    ...
    return {
      systemPrompt:
        dbPrompt.system_prompt || "You are an expert financial writer.",
...
async function generateWithAI(
  ...
  try {
    ...
    const prompts = await getArticlePrompt(
      topic,
      category,
      contentType,
      subcategory,
    ...
    logFn(
      `📝 Using dynamic prompts - Writer: ${category || "default"}, Category: ${category || "investing-basics"}, Subcategory: ${subcategory || "none"}, Content-Type: ${contentType}`,
    );
    ...
    logFn(
      `❌ Generation failed: ${error.message} (First 100 chars: ${error.message?.substring(0, 100)})`,
    );
    ...
    try {
      ...
      let fallbackContent =
        result?.content || result?.article || result?.text || result?.response;
      ...
      if (fallbackContent.trim().length < 100) {
        throw new Error(
          `Fallback returned insufficient content (${fallbackContent.length} chars)`,
        );
      }
      ...
      throw new Error(
        `All AI providers failed. Original: ${error.message}. Fallback: ${fallbackError.message}`,
      );
    }
  }
}
...
function detectSubcategory(
  ...
  if (category === "credit-cards") {
    ...
    if (text.includes("cashback") || text.includes("cash back"))
      return "cashback";
    if (text.includes("premium") || text.includes("concierge"))
      return "premium";
    ...
    if (
      ...
      return "fuel";
    if (text.includes("free") || text.includes("lifetime free"))
  ...
  if (category === "mutual-funds") {
    ...
    if (text.includes("large cap") || text.includes("large-cap"))
      return "large-cap";
    if (text.includes("mid cap") || text.includes("mid-cap")) return "mid-cap";
    if (text.includes("small cap") || text.includes("small-cap"))
      return "small-cap";
  }
  ...
  if (category === "loans") {
    ...
    if (
      ...
      return "car";
    if (text.includes("education") || text.includes("student"))
      return "education";
  }
...
export async function generateArticleCore(
  ...
  const { triggerArticlePublishingWorkflow } =
    await import("@/lib/workflows/hooks/article-workflow-hooks");
  ...
  try {
    const { checkForDuplicates } =
      await import("../quality/duplicate-detector");
    ...
    if (dupCheck.recommendation === "BLOCK") {
      ...
      if (jobId) {
        await storeJobStatus(
          jobId,
          "failed",
          { reason: dupCheck.reason, type: "duplicate_content" },
          "duplicate-check",
        );
      }
  ...
  function getDynamicQualityThreshold(
    topicStr: string,
    kd: number,
    attempt: number,
  ): number {
    ...
    if (
      /crash|correction|rally|rbi.*(cut|hike|rate)|budget|nifty|market down|rupee/i.test(
        t,
      )
    )
      base = 55;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      try {
        ...
        logFn(
          `   🎯 Target Word Count: ${brief.recommended_word_count}+ words`,
        );

        // NEW: Score keyword difficulty (graceful fallback)
        try {
          const { scoreKeywordDifficulty } =
            await import("@/lib/seo/keyword-difficulty-scorer");
          ...
          logFn(
            `   🎯 Keyword Difficulty: ${difficulty.level} (${difficultyScore}/100)`,
          );
      ...
      const enhancedBrief =
        attempt > 1 ? enhanceForUniqueness(brief, attempt) : brief;
      ...
      try {
        ...
        if (parsed) {
          ...
          const content =
            parsed.content || parsed.article || parsed.text || parsed.response;
          ...
          logFn(
            "   ⚠️ AI returned raw HTML/Text instead of JSON. Proceeding with raw content.",
          );
        }
      } catch (e: any) {
      ...
      // (Category-aware interlinking runs after title/slug/category detection below)
      ...
      const serpCompetitorAvg =
        enhancedBrief?.recommended_word_count ||
        brief?.recommended_word_count ||
        brief?.avg_word_count;
      const serpDominationTarget = serpCompetitorAvg
        ? Math.ceil(serpCompetitorAvg * 1.2)
        : 0;

      // Fallback floor based on content type
      const contentFloor =
        WORD_COUNT_FLOORS[contentType] || WORD_COUNT_FLOORS["default"];
      ...
      if (actualWordCount < minimumAcceptable) {
        ...
        const percentAchieved = Math.round(
          (actualWordCount / targetWordCount) * 100,
        );

        logFn(
          `   ⚠️ WARNING: Content is ${shortfall} words short! (${percentAchieved}% of target)`,
        );
      ...
      logFn(
        `   ✅ Word count validation passed (${Math.round((actualWordCount / targetWordCount) * 100)}% of target)`,
      );

      logFn(
        `   ✅ Generated ${html.length} characters, ${actualWordCount} words`,
      );
      ...
      const slug =
        ...
        "article-" + Date.now();
      const excerpt =
        aiSeoDesc || firstPara.substring(0, 160) || "Financial guide";
      ...
      // 4b. Category-Aware Interlinking (calculators, related articles, products, glossary)
      logFn("4️⃣b Category-aware interlinking...");
      try {
        const { generateInterlinks } = await import("@/lib/ai/auto-interlink");
        const interlinks = await generateInterlinks(category, title, slug);
        const linkSections: string[] = [];
        if (interlinks.calculatorLinks.length > 0) {
          linkSections.push(
            '<div class="related-tools"><h3>Related Calculators</h3><ul>' +
              interlinks.calculatorLinks
                .map(
                  (c) =>
                    `<li><a href="${c.path}">${c.name}</a> — ${c.context}</li>`,
                )
                .join("") +
              "</ul></div>",
          );
        }
        if (interlinks.relatedArticles.length > 0) {
          linkSections.push(
            '<div class="related-reading"><h3>Related Reading</h3><ul>' +
              interlinks.relatedArticles
                .map(
                  (a) =>
        ...
        if (interlinks.productPageLinks.length > 0) {
          linkSections.push(
            '<div class="related-products"><h3>Compare Products</h3><ul>' +
              interlinks.productPageLinks
                .map((p) => `<li><a href="${p.path}">${p.name}</a></li>`)
                .join("") +
              "</ul></div>",
          );
        }
        if (linkSections.length > 0) {
          html += "\n" + linkSections.join("\n");
          logFn(
            `   ✅ Added ${interlinks.calculatorLinks.length} calculators, ${interlinks.relatedArticles.length} articles, ${interlinks.productPageLinks.length} products`,
          );
        } else {
          logFn("   ℹ️ No interlinks found for this category");
        }
      } catch (interlinkError) {
        logFn("   ⚠️ Category interlinking failed, proceeding without.");
      }
      ...
      const imageResult = await imageService.getFeaturedImage(
        aiImageKeywords,
        category,
      );
      ...
      logFn(
        `   📊 Quality Score: ${qualityScore.total_score}/100 (${qualityScore.grade})`,
      );

      // Plagiarism check (FIXED: Corrected function call)
      const uniquenessCheck = await checkPlagiarism(
        html,
        topic,
        undefined,
        getSupabaseClient(),
      ...
      const QUALITY_THRESHOLD = getDynamicQualityThreshold(
        topic,
        difficultyScore,
        attempt,
      );
      logFn(
        `   \uD83C\uDFAF Dynamic Quality Threshold: ${QUALITY_THRESHOLD}/100 (attempt ${attempt}/${MAX_RETRIES})`,
      );
      const qualityFailed = qualityScore.total_score < QUALITY_THRESHOLD;
      const eeatFailed = qualityScore.eeat_score < EEAT_THRESHOLD;
      const plagiarismFailed =
        uniquenessCheck.isPlagiarized &&
        uniquenessCheck.similarityScore > PLAGIARISM_THRESHOLD;

      if (qualityFailed) {
        logFn(
          `   ⚠️ WARNING: Quality score below threshold (${qualityScore.total_score}/${QUALITY_THRESHOLD}).`,
        );
      }
      if (eeatFailed) {
        logFn(
          `   ⚠️ WARNING: E-E-A-T score below threshold (${qualityScore.eeat_score}/${EEAT_THRESHOLD}). Missing authority signals.`,
        );
      }
      if (plagiarismFailed) {
        logFn(
          `   ⚠️ WARNING: High plagiarism detected (${uniquenessCheck.similarityScore}% similarity).`,
        );
      }

      // Auto-correction: Retry if quality or plagiarism failed and we have retries left
      if (
        (qualityFailed || eeatFailed || plagiarismFailed) &&
        attempt < MAX_RETRIES
      ) {
        logFn(
          `   🔄 Auto-correction triggered. Regenerating article (attempt ${attempt + 1}/${MAX_RETRIES})...`,
        );
      ...
      if (
        ...
        logFn(
          `   🚨 FINAL WARNING: Quality/E-E-A-T/plagiarism issues remain after ${MAX_RETRIES} attempts.`,
        );
      ...
      const status =
        qualityFailed || eeatFailed || plagiarismFailed ? "draft" : "published";
      ...
      let resolvedAuthorId =
        (typeof options === "object" ? options?.authorId : undefined) || null;
      const resolvedAuthorName =
        (typeof options === "object" ? options?.authorName : undefined) ||
        "InvestingPro Team";
      ...
      if (!resolvedAuthorId) {
        try {
          const {
            data: { users },
            error,
          } = await getSupabaseClient().auth.admin.listUsers({
            page: 1,
            perPage: 1,
          });
        ...
        logFn(
          `   👤 Using selected author: ${resolvedAuthorName} (${resolvedAuthorId})`,
        );
      }
      ...
      const payload = {
        ...
        is_verified_quality:
          qualityScore.total_score >= 70 && !uniquenessCheck.isPlagiarized,
      ...
      try {
        ...
        logFn(
          `   🔄 Workflow triggered: Content generation workflow started for article ${article.id}`,
        );
      } catch (workflowError) {
        // Don't fail article creation if workflow fails
        logFn(
          `   ⚠️  Workflow trigger failed (non-critical): ${workflowError instanceof Error ? workflowError.message : String(workflowError)}`,
        );
      }
      ...
      logFn(
        `   ❌ Generation failed (attempt ${attempt}/${MAX_RETRIES}): ${error.message}`,
      );
