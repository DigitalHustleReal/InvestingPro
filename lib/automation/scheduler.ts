...
export async function cancelScheduledArticle(
    ...
    try {
        ...
        logger.error('Error cancelling scheduled article', error as Error, { articleId });
        return {
            success: false,
            error: error instanceof Error ? error.message : 'Unknown error'
        };
...
export async function getScheduledArticles(
    ...
    try {
        ...
        return (data || []).map((article: any) => ({
            ...
            status: (article.status || 'scheduled') as 'scheduled' | 'published' | 'cancelled',
...
export async function publishScheduledArticles(): Promise<{
    ...
    try {
        ...
        if (fetchError || !scheduledArticles) {
            logger.error('Failed to fetch scheduled articles for publishing', fetchError as Error);
            return results;
        }
