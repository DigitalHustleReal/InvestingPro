...
export class CreditCardDecisionEngine {
    ...
    return this.cards
      .map((card) => {
        ...
        if (card.type === "Cashback" || card.type === "cashback") {
          ...
          reasons.push(
            `Earn ₹${estimatedRewards.toFixed(0)}/month cashback on your spending`,
          );
          ...
          if (travelSpending > 5000) {
            reasons.push(
              `Ideal for frequent travelers (₹${travelSpending}/month on travel)`,
            );
          }
          ...
          score +=
            shoppingSpending > 10000
              ? estimatedRewards * 12
              : estimatedRewards * 5;
          if (shoppingSpending > 10000) {
            reasons.push(
              `Maximize rewards on online shopping (₹${shoppingSpending}/month)`,
            );
          }
        } else if (
          (card.type as string) === "Fuel" ||
          (card.type as string) === "fuel"
        ) {
          ...
          score +=
            fuelSpending > 3000 ? estimatedRewards * 15 : estimatedRewards * 5;
  ...
  ): Array<{ card: CreditCard; probability: number }> {
    return this.cards
      .map((card) => ({
        card,
  ...
  private extractAnnualFee(card: CreditCard): number {
    ...
    if (feeStr.includes("free") || feeStr === "0") return 0;
  ...
  private extractMinIncome(card: CreditCard): number {
    ...
    const match = incomeStr.match(
      /₹?\s*(\d+(?:,\d+)?)\s*\/?\s*(?:month|monthly|per month)/,
    );
    if (match) {
      return parseFloat(match[1].replace(/,/g, ""));
    }
    // Try annual
    const annualMatch = incomeStr.match(
      /₹?\s*(\d+(?:,\d+)?)\s*\/?\s*(?:year|annual|per year)/,
    );
    if (annualMatch) {
