...
export interface WhatIfScenario {
  type: 'spending_change' | 'income_change' | 'credit_score_change' | 'close_card' | 'new_card';
  change: any;
}
...
export class BreakEvenCalculator {
   ...
   */
  static calculate(card: CreditCard, profile: UserSpendingProfile): BreakEvenResult {
    const annualFee = this.parseAnnualFee(card);
    const rewardBreakdown = this.calculateRewardBreakdown(card, profile);
    const estimatedAnnualRewards = rewardBreakdown.reduce((sum, r) => sum + r.rewards, 0);
    
    // Add additional benefits value (lounge access, insurance, etc.)
    const additionalBenefitsValue = this.estimateAdditionalBenefits(card, profile);
    const totalValue = estimatedAnnualRewards + additionalBenefitsValue;
    
    const netBenefit = totalValue - annualFee;
    const breakEvenMonths = annualFee > 0 ? Math.ceil(annualFee / (totalValue / 12)) : 0;
  ...
  private static parseAnnualFee(card: CreditCard): number {
    ...
    if (feeStr.includes('free') || feeStr.includes('nil') || feeStr === '0') return 0;
  ...
  private static calculateRewardBreakdown(
    ...
    const categories = [
      ...
      { key: 'entertainment', name: 'Entertainment', rate: rewardRates.entertainment },
      { key: 'other', name: 'Other Spending', rate: rewardRates.other },
    ];
    
    for (const cat of categories) {
      const spending = profile.categories[cat.key as keyof typeof profile.categories] * 12;
      if (spending > 0) {
        breakdown.push({
          category: cat.name,
          spending,
  ...
  private static getRewardRates(card: CreditCard, cardType: string): Record<string, number> {
    // Base rates
    let rates = {
      online: 1,
      travel: 1,
    ...
    const cardText = `${card.rewards?.join(' ')} ${card.description || ''}`.toLowerCase();
    
    const rateMatch = cardText.match(/(\d+(?:\.\d+)?)\s*%?\s*(?:cashback|rewards?)/);
    if (rateMatch) {
  ...
  private static estimateAdditionalBenefits(card: CreditCard, profile: UserSpendingProfile): number {
    let value = 0;
...
export class ApprovalProbabilityCalculator {
   ...
   */
  static calculate(card: CreditCard, profile: UserSpendingProfile): ApprovalProbabilityResult {
    ...
    const creditFactor = this.assessCreditScore(profile.creditScore, minCreditScore);
    factors.push(creditFactor);
    baseProbability += creditFactor.impact;
    
    // 3. Employment Type Assessment
    const employmentFactor = this.assessEmployment(profile.employmentType, card);
    ...
    const recommendations = this.generateRecommendations(factors, overallProbability);
  ...
  private static parseMinIncome(card: CreditCard): number {
    ...
    // Try monthly
    let match = incomeStr.match(/₹?\s*(\d+(?:,\d+)?)\s*(?:\/?\s*(?:month|monthly|per month|pm))/);
    if (match) return parseFloat(match[1].replace(/,/g, ''));
    
    // Try annual
    match = incomeStr.match(/₹?\s*(\d+(?:,\d+)?)\s*(?:\/?\s*(?:year|annual|pa|per annum))/);
  ...
  private static assessIncome(userIncome: number, minIncome: number): ApprovalFactor {
  ...
  private static assessCreditScore(
    ...
    if (!userScore) {
      return {
        ...
        suggestion: 'Check your credit score on CIBIL or Experian for accurate assessment.',
      };
    }
    ...
    if (userScore >= 750) {
      ...
      return {
        ...
        currentValue: `${userScore}`,
        requiredValue: `${minScore}+`,
        suggestion: 'Score is acceptable but improving to 750+ increases chances.',
      };
    } else {
      return {
        name: 'Credit Score',
  ...
  private static assessExistingCards(count: number): ApprovalFactor {
    if (count === 0) {
      return {
        ...
        requiredValue: 'N/A',
        suggestion: 'First card applicants may need stronger income/employment proof.',
      };
    } else if (count <= 2) {
      return {
        name: 'Existing Credit Cards',
      ...
      return {
        ...
        currentValue: `${count} cards`,
        requiredValue: 'N/A',
        suggestion: 'High card count may raise concerns. Consider consolidating.',
      };
    }
  }
  ...
  private static assessCardType(card: CreditCard): ApprovalFactor {
    ...
    if (type.includes('premium') || type.includes('super premium')) {
      return {
        ...
        suggestion: 'Premium cards have stricter criteria. Ensure strong profile.',
      };
  ...
  private static assessLocation(city: UserSpendingProfile['city']): ApprovalFactor {
    switch (city) {
      case 'metro':
        return {
          name: 'Location',
  ...
  private static generateRecommendations(
    ...
    if (probability < 40) {
      recommendations.push('Consider applying for an entry-level or secured card first.');
      recommendations.push('Build credit history for 6-12 months before reapplying.');
    } else if (probability < 60) {
      recommendations.push('Apply through pre-approved offers if available.');
      recommendations.push('Have all documents ready: salary slips, bank statements, ID proof.');
...
export class CardComboOptimizer {
  ...
  static optimize(
    ...
    const totalAnnualValue = usageStrategy.reduce((sum, s) => sum + s.estimatedAnnualRewards, 0);
    ...
    const recommendation = additionalValue > 5000
  ...
  private static findCategoryBestCards(
    ...
    const categories: Array<{ key: keyof typeof profile.categories; name: string }> = [
  ...
  private static getCardRewardRateForCategory(card: CreditCard, category: string): number {
    const type = (card.type || '').toLowerCase();
  ...
  private static selectOptimalCombo(
    ...
    const cardValues = new Map<string, { card: CreditCard; totalValue: number; categories: string[] }>();
    ...
    for (const [category, { card, value }] of categoryBest) {
      ...
      if (existing) {
        ...
        cardValues.set(card.id, { card, totalValue: value, categories: [category] });
      }
    }
    
    // Sort by total value and pick top cards
    const sorted = Array.from(cardValues.values())
...
export class DecisionMatrixGenerator {
   ...
   */
  static generate(cards: CreditCard[], profile?: UserSpendingProfile): DecisionMatrixData {
    const dimensions: MatrixDimension[] = [
      ...
      this.createDimension(cards, 'cost', 'Low Cost', 0.20),
  ...
  private static calculateDimensionScore(card: CreditCard, dimension: string): number {
    const type = (card.type || '').toLowerCase();
    ...
    switch (dimension) {
      case 'travel':
        ...
        if (card.loungeAccess && !card.loungeAccess.toLowerCase().includes('nil')) {
          travelScore += 30;
        }
        if (featuresText.includes('forex') || featuresText.includes('international')) {
          travelScore += 10;
        }
      ...
      case 'shopping':
        let shopScore = 30;
        if (type.includes('shopping') || type.includes('cashback')) shopScore += 40;
        if (featuresText.includes('amazon') || featuresText.includes('flipkart')) {
          shopScore += 20;
        }
        if (featuresText.includes('emi') || featuresText.includes('no cost emi')) {
          shopScore += 10;
        }
      ...
      case 'premium':
        let premiumScore = 20;
        if (type.includes('premium') || type.includes('super premium')) premiumScore += 50;
        if (card.loungeAccess?.toLowerCase().includes('unlimited')) premiumScore += 20;
        if (featuresText.includes('concierge')) premiumScore += 10;
        if (featuresText.includes('insurance') || featuresText.includes('cover')) premiumScore += 10;
  ...
  private static findUserBestMatch(
    ...
    const totalSpending = Object.values(profile.categories).reduce((a, b) => a + b, 0);
    const userWeights: Record<string, number> = {
      travel: profile.categories.travel / totalSpending,
      shopping: (profile.categories.online + profile.categories.groceries) / totalSpending,
...
export class WhatIfSimulator {
  ...
  static simulate(
    ...
    const modifiedProfile = { ...currentProfile, categories: { ...currentProfile.categories } };
    ...
    switch (scenario.type) {
      case 'spending_change':
        return this.simulateSpendingChange(scenario, cards, currentProfile, modifiedProfile);
        
      case 'income_change':
        return this.simulateIncomeChange(scenario, cards, currentProfile, modifiedProfile);
        
      case 'credit_score_change':
        return this.simulateCreditScoreChange(scenario, cards, currentProfile, modifiedProfile);
  ...
  private static simulateSpendingChange(
    ...
    const { category, newAmount } = scenario.change as { category: string; newAmount: number };
    const currentAmount = currentProfile.categories[category as keyof typeof currentProfile.categories];
    
    modifiedProfile.categories[category as keyof typeof modifiedProfile.categories] = newAmount;
    ...
    if (cardChanged) {
      ...
      const currentRewards = BreakEvenCalculator.calculate(currentBest, currentProfile).estimatedAnnualRewards;
      const modifiedRewards = BreakEvenCalculator.calculate(currentBest, modifiedProfile).estimatedAnnualRewards;
  ...
  private static simulateIncomeChange(
    ...
    const newlyEligible = cards.filter(c => {
      const result = ApprovalProbabilityCalculator.calculate(c, modifiedProfile);
      return result.overallProbability >= 50;
    });
    ...
    if (newCards.length > 0) {
      impact = `${newCards.length} new premium cards become accessible: ${newCards.slice(0, 3).map(c => c.name).join(', ')}`;
      ...
      impact = 'Income increase improves your approval odds for existing card options.';
      recommendation = 'Your approval probability has improved across all cards.';
    } else {
      impact = 'Some premium cards may become less accessible.';
      recommendation = 'Focus on cards within your new income range.';
      warnings.push('Lower income may affect credit limit on new applications.');
    }
    
    return {
      scenario,
      currentState: { income: currentProfile.monthlyIncome, eligibleCards: currentlyEligible.length },
      projectedState: { income: newIncome, eligibleCards: newlyEligible.length },
      impact,
      recommendation,
      warnings,
    };
  ...
  private static simulateCreditScoreChange(
    ...
    const premiumCard = cards.find(c => 
      (c.type || '').toLowerCase().includes('premium')
    ) || cards[0];
    
    const currentProb = ApprovalProbabilityCalculator.calculate(premiumCard, currentProfile);
    const newProb = ApprovalProbabilityCalculator.calculate(premiumCard, modifiedProfile);
    
    const probDiff = newProb.overallProbability - currentProb.overallProbability;
    ...
    if (probDiff > 0) {
      ...
      recommendation = newProb.overallProbability >= 70
      ...
      warnings.push('Lower credit score significantly impacts premium card approval.');
    }
    
    return {
      scenario,
      currentState: { creditScore: currentProfile.creditScore || 'Unknown', approvalOdds: currentProb.overallProbability },
      projectedState: { creditScore: newScore, approvalOdds: newProb.overallProbability },
      impact,
      recommendation,
      warnings,
    };
  ...
  private static simulateCloseCard(
    ...
    if (cardAge > 5) {
      warnings.push('This is your oldest card. Closing it will significantly impact credit history length.');
      recommendation = 'NOT RECOMMENDED. Keep this card active even with minimal use.';
    } else if (cardAge > 2) {
      warnings.push('Closing this card will reduce your average account age.');
      recommendation = 'Consider keeping the card. Use it once every few months to keep it active.';
    } else {
      recommendation = 'Impact is minimal if you have other older cards. Safe to close if needed.';
    }
  ...
  private static simulateNewCard(
    ...
    const approval = ApprovalProbabilityCalculator.calculate(card, currentProfile);
    ...
    if (!breakEven.isWorthIt) {
      warnings.push('Card may not provide positive ROI based on your spending.');
    }
    if (currentProfile.existingCards >= 5) {
      warnings.push('You have multiple cards. New application may face scrutiny.');
    }
  ...
  private static findBestCard(cards: CreditCard[], profile: UserSpendingProfile): CreditCard {
...
export const SmartComparison = {
  ...
  calculateApprovalProbability: ApprovalProbabilityCalculator.calculate.bind(ApprovalProbabilityCalculator),
  optimizeCardCombo: CardComboOptimizer.optimize.bind(CardComboOptimizer),
  generateDecisionMatrix: DecisionMatrixGenerator.generate.bind(DecisionMatrixGenerator),
