...
const groq =
  typeof window === "undefined" && process.env.GROQ_API_KEY
    ? new Groq({ apiKey: process.env.GROQ_API_KEY })
    : null;

const mistral =
  typeof window === "undefined" && process.env.MISTRAL_API_KEY
    ...
    if (data && !error) {
      data.forEach((p: any) => {
        providerHealth[p.provider_name] = {
          ...
          lastFailureTime: p.last_failure_time
            ? new Date(p.last_failure_time).getTime()
            : undefined,
        };
      });
...
function checkHealth(name: string): boolean {
  ...
  if (
    ...
    Date.now() - health.lastFailureTime > COOLDOWN_PERIOD
  ) {
...
async function reportFailure(name: string, error: string) {
  ...
  if (health.failureCount >= CIRCUIT_BREAKER_THRESHOLD) {
    ...
    logger.error(
      `CIRCUIT BREAKER: Provider ${name} marked as DEGRADED due to repeated failures.`,
      new Error(error),
    );
  }
    ...
    Core: {
      ...
      InvokeLLM: async ({
        ...
        const finalSystemPrompt =
          customSystemPrompt || generateSystemPrompt(operation);
          ...
          try {
            const response = await openai.chat.completions.create({
              ...
              messages: [
                {
                  role: "system",
                  content: finalSystemPrompt + " You must respond in JSON.",
                },
                { role: "user", content: enhancedPrompt },
              ],
              response_format: { type: "json_object" },
              temperature: 0.3,
              max_tokens: 8000, // Supports NerdWallet-grade 2,500-5,000 word articles
            });
            ...
            if (rawContent) {
              ...
              if (contentToValidate && typeof contentToValidate !== "string") {
                logger.warn(
                  "OpenAI returned object/array for content, stringifying",
                );
        ...
        if (geminiKey && checkHealth("gemini")) {
          try {
            ...
            const { fetchWithTimeout } =
              await import("@/lib/utils/fetch-with-timeout");
      ...
      UploadFile: async ({ file }: { file: File }) => {
        ...
        if (isImage) {
          try {
            ...
            logger.warn(
              "Optimized upload failed, using direct upload",
              optimizeError as Error,
            );
        ...
        try {
          ...
          logger.warn(
            "Media registered in Storage but failed to record in DB table.",
          );
        }
  ...
  entities: {
    ...
    Article: {
      ...
      create: async (data: any) => {
        ...
        if (!response.ok) {
          const error = await response
            .json()
            .catch(() => ({ message: "Failed to create article" }));
    ...
    MutualFund: {
      list: async (
        ...
        if (searchTerm) {
          query = query.or(
            `name.ilike.%${searchTerm}%,fund_house.ilike.%${searchTerm}%`,
          );
        }
