...
export class RevenueService {
    ...
    async getDashboardMetrics(startDate?: string, endDate?: string): Promise<RevenueMetrics> {
        // Validation logic moved here or kept in controller
        ...
        const { data: currentRevenueData } = await this.supabase
        ...
        const currentRevenue = currentRevenueData?.reduce((sum: number, row: any) => sum + (Number(row.commission_earned) || 0), 0) || 0;
        ...
        const previousRevenue = previousRevenueData?.reduce((sum: number, row: any) => sum + (Number(row.commission_earned) || 0), 0) || 0;

        // Calculate growth percentage
        const growth = previousRevenue > 0 ? ((currentRevenue - previousRevenue) / previousRevenue) * 100 : 0;

        // Revenue by category (using injected range or current month defaults - mirroring original logic)
        // Original logic used current month for specific metrics to be consistent
        ...
        // Conversion rates
        const { data: allClicks } = await this.supabase
            .from('affiliate_clicks')
        ...
        const creditCardClicks = allClicks?.filter((c: any) => 
            c.product_type?.toLowerCase().includes('credit_card') || 
            c.product_type?.toLowerCase().includes('creditcard')
        ) || [];
        const creditCardConversions = creditCardClicks.filter((c: any) => c.converted).length;
        ...
        const mutualFundClicks = allClicks?.filter((c: any) => 
        ...
        const mutualFundConversions = mutualFundClicks.filter((c: any) => c.converted).length;
        ...
        for (let i = 29; i >= 0; i--) {
            ...
            const dayRevenueTotal = dayRevenue?.reduce((sum: number, row: any) => sum + (Number(row.commission_earned) || 0), 0) || 0;
        ...
        for (let i = 11; i >= 0; i--) {
            ...
            const { data: weekRevenue } = await this.supabase
                .from('affiliate_clicks')
                .select('commission_earned')
                .eq('converted', true)
            ...
            const weekRevenueTotal = weekRevenue?.reduce((sum: number, row: any) => sum + (Number(row.commission_earned) || 0), 0) || 0;
        ...
        for (let i = 11; i >= 0; i--) {
            ...
            const { data: monthRevenue } = await this.supabase
                .from('affiliate_clicks')
                .select('commission_earned')
                .eq('converted', true)
            ...
            const monthRevenueTotal = monthRevenue?.reduce((sum: number, row: any) => sum + (Number(row.commission_earned) || 0), 0) || 0;
