...
export interface ArticleQuery {
  ...
  status?: string;
  search?: string;
  fields?: string; // Supabase select fields — defaults to listing fields (no body_html)
}
  ...
  async findMany(query: ArticleQuery): Promise<{ data: any[]; count: number }> {
    ...
    try {
      let dbQuery = supabase
        ...
        .select(
          query.fields ||
            "id, title, slug, excerpt, category, featured_image, author_name, author_role, read_time, reading_time, published_at, published_date, updated_at, tags, status, views",
          { count: "exact" },
        )
      ...
      if (error) {
        ...
        try {
          ...
          logger.debug("RPC fallback failed", {
            error:
              rpcFallbackError instanceof Error
                ? rpcFallbackError.message
                : String(rpcFallbackError),
          });
        }
      ...
      logger.error(
        "Article repository findMany error",
    ...
    return await withPerformanceTracking(
      ...
      async () => {
        ...
        try {
          ...
          logger.error(
            "Article repository findById error",
            error instanceof Error ? error : new Error(String(error)),
          );
          throw error;
        }
    ...
    return await withPerformanceTracking(
      ...
      async () => {
        ...
        try {
          ...
          logger.error(
            "Article repository findBySlug error",
            error instanceof Error ? error : new Error(String(error)),
          );
          throw error;
        }
