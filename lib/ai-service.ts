...
import {
  BudgetGovernorAgent,
  BudgetStatus,
} from "./agents/budget-governor-agent";
...
class AIService {
  ...
  private initializeProviders() {
    ...
    if (this.providers.length === 0) {
      ...
      throw new Error(
        "AI Service requires at least one provider. Check your .env.local file.",
      );
    }
  ...
  async generate(
    ...
    if (this.budgetCheckEnabled && !skipBudgetCheck) {
      try {
        ...
        throw new Error(
          "Budget verification failed. Circuit closed for safety.",
        );
      }
    }
    ...
    if (tier === "draft") {
      ...
      preferredProviders.sort((a, b) => {
        ...
        if (cheapOnes.includes(a.name) && !cheapOnes.includes(b.name))
          return -1;
    ...
    if (finalPrompt.length > 5000) {
      finalPrompt = finalPrompt.replace(
        /ROLE: You are "Vikram Mehta"[\s\S]*?(?=GOAL:)/,
        "ROLE: Senior Wealth Advisor (Vikram Mehta). Style: Authoritative, conversational, Indian context. ",
      );
    }
    ...
    for (const provider of preferredProviders) {
      ...
      if (!provider.isAvailable() || circuitBreaker.getState() === "open")
        continue;
      ...
      try {
        ...
        this.trackCost(response, articleId, operation, duration).catch((e) =>
          logger.warn("Cost log fail", e),
        );
  ...
  async generateProduct(name: string, category: string) {
    ...
    try {
      const { getPromptWithFallback } =
        await import("@/lib/ai/prompts-service");
...
`;
  ...
  async generateArticle(topic: string, keywords: string[], category?: string) {
    ...
    try {
      ...
      const { detectIntent, CATEGORY_CONFIGS } =
        await import("@/lib/content/content-schema");
      const { getAuthorForCategory, getAuthorSystemPrompt } =
        await import("@/lib/content/author-personas");
      ...
      result[providerName] = {
        ...
        successRate:
          metrics.totalCalls > 0
            ? Math.round(
                (metrics.successCount / metrics.totalCalls) * 100 * 100,
              ) / 100
            : 100,
        avgLatencyMs:
