...
export class ArticleService {
  ...
  private getClient() {
    return this.customClient || createServiceClient();
  }
  ...
  async saveArticle(
    ...
    if (metadata.tags) {
      metadata.tags = Array.from(
        new Set(metadata.tags.map((t) => t.trim().toLowerCase())),
      ).filter(Boolean);
    }
    ...
    try {
      const markdownBeforeDisclosure = normalizedHTML
        ? htmlToMarkdown(normalizedHTML)
        : content.body_markdown || content.content || "";
      const markdownWithDisclosure = injectAffiliateDisclosure(
        markdownBeforeDisclosure,
        metadata.category,
      );
      ...
      if (markdownWithDisclosure !== markdownBeforeDisclosure) {
        ...
        const htmlWithDisclosure = markdownWithDisclosure
          ? normalizeArticleBody(markdownWithDisclosure)
          : normalizedHTML;
        normalizedHTML = htmlWithDisclosure;
        logger.info("Affiliate disclosure injected during save", {
          articleId: id,
      ...
      logger.warn("Failed to inject affiliate disclosure", {
        articleId: id,
        error:
          disclosureError instanceof Error
            ? disclosureError.message
            : String(disclosureError),
      });
    }

    const normalizedMarkdown = normalizedHTML
      ? htmlToMarkdown(normalizedHTML)
      : "";
    ...
    if (metadata.status && metadata.status !== previousStatus) {
      try {
        ...
        if (
          ...
          workflowError.message.includes("server-only")
        ) {
          logger.debug("State transition skipped (client context)");
        } else {
          logger.error(
            "Failed to trigger state transition",
            workflowError instanceof Error
              ? workflowError
    ...
    try {
      const { createArticleVersion } =
        await import("@/lib/cms/version-service");
  ...
  async quickSaveMetadata(
    ...
    if (metadata.status && metadata.status !== previousStatus) {
      try {
        ...
        logger.warn("Workflow transition skipped or failed in quickSave", {
          articleId: id,
        });
      }
    }
  ...
  async publishArticle(
    ...
    try {
      const { autoInterlinkArticle } =
        await import("@/lib/automation/auto-interlinking");
      autoInterlinkArticle(id, { maxLinks: 5, minRelevance: 0.3 }).catch(
        (err) => {
      ...
      logger.warn("Auto-interlinking setup failed", {
        articleId: id,
        error:
          interlinkError instanceof Error
            ? interlinkError.message
            : String(interlinkError),
      });
    }
  ...
  async createArticle(
    ...
    let slug = metadata.slug
      ? this.normalizeSlug(metadata.slug)
      : this.generateSlug(metadata.title);
    ...
    // 3. Category Validation & Normalization
    metadata.category = (metadata.category || "investing-basics")
      .trim()
      .toLowerCase();
  ...
  async listArticles(
    ...
    const cacheKey = cacheKeyGenerators.article.list(
      `limit:${limit || "all"}:deleted:${includeDeleted}`,
    );
    ...
    if (error) {
      ...
      logger.info(
        `[ArticleService] Direct query returned ${data.length} articles`,
      );
      ...
      if (!rpcError && rpcData) {
        ...
        if (rpcArticles.length > 0) {
          articles = rpcArticles.map((item: any) =>
        ...
        const { data: altRpcData, error: altRpcError } = await supabase.rpc(
        ...
        if (!altRpcError && altRpcData && altRpcData.length > 0) {
          articles = (altRpcData as any[]).map((item) =>
            this.normalizeArticle(item),
          );
        }
  ...
  private normalizeArticle(data: any): ArticleData {
    return {
      ...
      author_name:
        data.authors?.name || data.author_name || "InvestingPro Team",
