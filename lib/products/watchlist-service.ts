...
export async function addToWatchlist(item: WatchlistItem): Promise<{ success: boolean; id?: string; error?: string }> {
...
export async function removeFromWatchlist(user_id: string, product_id: string): Promise<boolean> {
  const supabase = createClient()
...
export async function getUserWatchlist(user_id: string): Promise<WatchlistItem[]> {
  const supabase = createClient()
...
export async function createPortfolio(portfolio: SavedPortfolio): Promise<{ success: boolean; id?: string; error?: string }> {
  const supabase = createClient()

  const total_weight = portfolio.portfolio_items.reduce((sum, i) => sum + i.weight_pct, 0)
  if (Math.abs(total_weight - 100) > 0.5) {
    return { success: false, error: `Portfolio weights must sum to 100% (got ${total_weight}%)` }
  }

  const share_token = portfolio.is_public
    ? generateShareToken()
    : undefined
...
export async function getUserPortfolios(user_id: string): Promise<SavedPortfolio[]> {
  const supabase = createClient()
...
export async function getPortfolioByShareToken(token: string): Promise<SavedPortfolio | null> {
...
export async function deletePortfolio(id: string, user_id: string): Promise<boolean> {
  const supabase = createClient()
  const { error } = await supabase
    .from('saved_portfolios')
    .delete()
...
export function analyzePortfolio(
  ...
  products: Array<{ id: string; returns_3y?: number; risk_level?: string; category?: string }>
): PortfolioAnalysis {
  ...
  for (const item of items) {
    ...
    else if (risk === 'high' || risk === 'very_high') risk_breakdown.high += item.weight_pct
  ...
  const corpus = (n: number) => total_monthly * ((Math.pow(1 + r, n) - 1) / r) * (1 + r)

  const categories = Object.keys(category_breakdown).length
  const diversification_score = Math.min(100, categories * 20 +
    (risk_breakdown.moderate > 30 && risk_breakdown.moderate < 70 ? 20 : 0) +
    (risk_breakdown.high < 60 ? 10 : 0))

  const recommendations: string[] = []
  if (risk_breakdown.high > 70) recommendations.push('Consider adding debt funds for stability')
  if (categories < 2) recommendations.push('Diversify across more asset classes')
  if (!items.some(i => i.category === 'insurance')) recommendations.push('Add term insurance to protect this portfolio')
