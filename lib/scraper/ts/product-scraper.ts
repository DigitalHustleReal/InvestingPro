...
export class ProductScraperVideo {
  ...
  async scrapeCreditCard(url: string) {
    ...
    const feeText = $(
      'span:contains("Annual Fee"), span:contains("Joining Fee")',
    )
      .first()
      .text();
    const annualFee = this.extractPrice(feeText);

    const rewardText = $('div:contains("Reward"), div:contains("Cashback")')
      .first()
      .text();
    const rewardRate = this.extractPercentage(rewardText);

    const features: string[] = [];
    $("li.feature-item, div.feature")
      .slice(0, 10)
      .each((_, el) => {
  ...
  async updateProduct(data: any, type: "credit_card" | "mutual_fund") {
    ...
    const productRecord: ProductInsert = {
      ...
      name: data.name,
      category: type,
      provider_name: this.extractProvider(data.name, data.source_url),
      is_active: true,
